...
function runtimeBootstrapCommand(workspaceMountPath: string): string {
  ...
  return [
    ...
    'BROWSER_PYTHON=/opt/ky-agent/browser-runtime/bin/python3',
    'if [ ! -e "$BROWSER_PYTHON" ]; then BROWSER_PYTHON="$(command -v python3)"; fi',
    'test -x "$BROWSER_PYTHON"',
    '"$BROWSER_PYTHON" - <<\\PY',
    ...
    '    browser = p.chromium.launch(headless=True, channel="chromium", args=["--disable-dev-shm-usage"])',
    '    browser.close()',
    '    print("BROWSER_RUNTIME_READY", executable)',
    'PY',
