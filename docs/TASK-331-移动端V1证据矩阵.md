...
> Review 返工映射：`31b44427a` 是 M00-01 Router/Gate A 支持提交，`d20fa2ccc` 是 M60-04 发布授权修复，`858c0659e` 是 M30-01 HTTP 后置异步竞态修复，`ed67c1f29` 是 M30-01 Web saved-account 可序列化切换修复。以上均为当前最终重放后的提交；本轮 `4ad679048`/`321f94e25` 修复身份顺序与缓存隔离，`ff537f858`/`6a0ad1cda` 修复 enterprise 凭据矩阵。
...
5. 下列定向用例数来自当前树实际执行；`pnpm mobile-contract` 共 **1,695** 项（Shared Vitest 1,301；Mobile Vitest 195；Mobile Node 199），另含 Shared/Mobile typecheck 与离线 Expo 检查。
...
| M10-02 | Expo SDK 55 依赖矩阵收口且双平台原生生成可解释 | 主体 `7d7e55dba4d8b0e2166cda0e8bf00952e1ce7055` | `mobile/package.json:52`；`pnpm-workspace.yaml:13` | `pnpm mobile-contract`：1,695/1,695；其中 `EXPO_OFFLINE=1 ... expo install --check` 退出 0 | 部分 | 在线检查发现 55.0.31，但受 `minimumReleaseAge: 2880`（48h cooling）未升级；离线结果不得写成在线 0 mismatch；仍需在线检查与双平台真实 clean prebuild 回执 |
...
| M60-04 | 同一已审 SHA 构建、验证、提交，产物绑定版本/hash/签名/SBOM/审批 | 主体 `c123f24d065217c417904d6a655582c33c3bc9b4`；支持 `d20fa2ccc3339847558fd317f45cd08cc86c4295`、`ff537f85813facf128f16826d9f27652cbc122c3`、`6a0ad1cda9aa6e60c5358f5761a34f653453560b` | `.github/workflows/mobile-submit.yml:1`；`mobile/scripts/mobile-release-evidence.test.mjs:1` | `pnpm -F mobile test:m60-04`：29/29 | 部分 | 组织 robot token、保护环境审批、EAS build ID、真实 IPA/AAB/APK、签名指纹、SHA-256、SBOM/provenance、submit 回执 |
...
| 验证项 | 当前事实 | 解释边界 |
|---|---|---|
| Mobile contract | `pnpm mobile-contract` 当前退出 0：Shared typecheck；Shared 100 files / 1,301 tests；Mobile typecheck；Mobile Vitest 42 files / 195 tests；Mobile Node 199 tests；合计 1,695 tests | 证明 Shared/Mobile 代码与契约当前通过；不证明生产服务、真机、签名、商店或灰度通过 |
...
1. **shared/mobile typecheck/test 连续稳定全绿**：当前 mobile-contract 1,695/1,695；但历史根并行 OAuth timeout/137，连续稳定性及 required-check 仍仅部分；生产 Gate **blocked**。
