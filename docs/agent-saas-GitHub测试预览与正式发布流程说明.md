...
> - 第十五轮返工最终同步基线：`main@837aa982740a65f1f1da94cb3a6bebc98f82021a`；最终实现以 PR #366 当前精确 head 与 Provider inspection 为准，避免在同一提交中写入不可能稳定的自引用 commit hash。
...
- Web identity 的 `configFingerprint/webDigest` 与 Manifest 一致。
- API ready 的 `serverDigest/webDigest/ACS Orchestrator/Sandbox digest` 与 Manifest 一致。
