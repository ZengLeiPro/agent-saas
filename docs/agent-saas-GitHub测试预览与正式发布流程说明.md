...
- 数据库迁移只允许 `none` 或 `expand`；`contract` 必须另发版本。migration 闭包同时跟踪静态 import 的 JSON/其他非代码资源和 `readFile/readFileSync(new URL(..., import.meta.url))` 资源；migration/governance-schema 下不能静态证明来源的动态资源路径 fail closed。
