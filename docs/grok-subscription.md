...
点击账号的“重授权”只能替换相同身份，保留优先级位置；Vault ref 可以变化。重复添加同一身份会被拒绝，应使用重授权。Access token 到期本身不意味着永久断开；共享管理器在到期前 10 分钟按需刷新。刷新链路上的连接级失败先在同一代理重试；仍失败时保留当前凭据、释放刷新 fence，下一次访问沿用同一 refresh token 重试，到期前继续使用旧 access token。只有授权服务器明确拒绝 grant（`invalid_grant`、`invalid_token`、`expired_token`、`unauthorized_client`、`invalid_client`）才标记永久失效并要求重授权；xAI 若已在上游消费该 grant，重试会得到明确拒绝，进入同一流程。
...
| `refresh_transient_failure`                          | 传输/上游/本地写入导致刷新未完成；账号未被拒绝，不需要重授权。下一次会话、目录或额度采集访问会自动重试；日志含脱敏原因 |
| `refresh_outcome_unknown`（旧版本遗留标记）          | 升级后自动视为可恢复：先清除标记并重试刷新；若上游返回 `invalid_grant` 才需重授权                                      |
| `invalid_grant` 等明确拒绝                           | 授权服务器已拒绝该 grant，重授权。套餐额度采集对该账号停打 billing，卡片标「凭据不可用，请重授权」                     |
