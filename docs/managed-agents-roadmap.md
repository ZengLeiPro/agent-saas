...
- **唯一 ready tenant hand 自动路由（B2）**：`selectRuntimeHandRoute()` 由 `WorkspaceToolProvider` 与 `RawAgentLoop` 共用，session 内仅 1 个 ready tenant-remote hand 时工具入参无显式 `handId` 也自动路由；effective handId 写入 `tool_invocation_started` metadata，`autoRoutedHandId` 让审计区分。
...
- [x] 默认路由：session 内唯一 ready tenant-remote hand 时自动路由。`selectRuntimeHandRoute()`（在 `handStore.ts`）由 `WorkspaceToolProvider` 与 `RawAgentLoop` 共用；判定条件 `status='ready' + type='server-remote' + metadata.tenantRemoteHandId` 存在。effective handId 写入 `tool_invocation_started` 的 metadata（同时记 `autoRoutedHandId` 让审计区分显式 vs 自动）。
