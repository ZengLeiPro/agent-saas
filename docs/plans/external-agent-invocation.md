...
> 状态：已完成（P0-P4 均已完成；待推送、部署及真实客户环境验收）
>
> 版本：V2（个人 Agent MVP）
>
> 创建日期：2026-09-15
...
实现说明：外部入口按 API Client 实施每分钟速率限制和进程内并发上限；模型执行继续复用运行时全局容量门禁。预算与硬封顶复用 API Client 所绑定专用账号的成员预算及组织余额策略，运营接口返回低余额/专用账号预算耗尽告警。费用以 Billing usage event、ledger 和 execution/run 关联为事实源，不重复建立平行计费账本。
...
API Client 管理端维护可调用的组织 Agent 白名单；创建 conversation 时重新校验组织归属、启用状态与 Client 授权，并将选定 Agent 固定到 conversation，后续消息不能切换。无 `agent_id` 的既有调用路径与个人 Agent 行为保持不变。
