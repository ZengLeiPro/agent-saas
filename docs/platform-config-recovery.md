...
浏览器或代理丢失响应时，先用同一登录身份查询 `/api/admin/config-operations/<operationId>`。`applied` 表示可刷新业务 GET 核对；`committed_unconfirmed` 表示签名 head 已提交但最终双端确认响应不完整；`publishing`/`preparing` 不得重发；`recovery_required` 先执行同 release 的受控恢复；只有 `not_committed` 或 `rolled_back` 在重新读取 raw revision、重新核对草稿并重新确认后，才可生成新的 operationId。状态查询自身网络失败、401/5xx 或返回未知状态时，前端必须保留并展示原 operationId，禁止生成新操作，直到状态查询成功或重新读取配置。不得删除 operation journal 来绕过状态。
