...
identity 还必须包含 `schemaVersion: 1`、`environment: production`、完整 `gitSha`，以及 `web`、`api`、`runtimeWorker`、`acs` 四个组件的完整 SHA 与 `deployedAt`。`releaseSymlink` 的 realpath 必须包含对应组件 SHA；pidfile、Worker readyfile 和 systemd MainPID 必须为同一存活进程。
