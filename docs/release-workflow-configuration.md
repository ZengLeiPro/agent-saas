...
Secrets：`ALIYUN_ACCESS_KEY_ID`、`ALIYUN_ACCESS_KEY_SECRET`、`ACR_READ_ACCESS_KEY_ID`、
`ACR_READ_ACCESS_KEY_SECRET`、`STAGING_ECS_HOST`、`STAGING_ECS_USER`、`STAGING_ECS_SSH_KEY`、
`RELEASE_EVIDENCE_TOKEN`、`STAGING_E2E_USERNAME`、`STAGING_E2E_PASSWORD`。

Staging RC 只在解析 ACS 镜像的步骤使用 `ACR_READ_ACCESS_KEY_ID` / `ACR_READ_ACCESS_KEY_SECRET`；
它们必须配置在 `staging` Environment，并保持与 production 中同名凭据相同的 ACR 只读权限，
不能把 Staging 通用 RAM 身份扩权为 ACR 读取身份。
...
ACS workflow 会遍历 ACR build-record API 的全部分页，用短 tag 筛选全局唯一候选，并从 build-record 日志 fetch 阶段的 `commit info` 行绑定源 commit（ACR 只打印 7 位缩写 SHA 与分支，日志中没有阶段字段和完整 SHA，因此校验为「缩写 ≥7 位且是请求 SHA 的前缀、分支为 main」）；分页总数因并发构建变化时会从第一页重新读取，连续三次仍无法取得稳定快照才 fail closed，记录缺失或重复也不会放行。解析 digest 前后还要再次遍历全部分页，确认候选 tag 仍只有一个固定的 `BuildRecordId`，并对 tag 做两次稳定读回。最终部署只使用 digest reference。ACR API 不直接返回 build-record digest，因此代码保证仍依赖现场 ACR 写权限只授予受控构建链，禁止人工或其他身份重定向候选 tag。

可选恢复 Secret：`ACS_WEBHOOK_REDELIVERY_TOKEN`。它只供 ACS compatibility 在当前 SHA 的 ACR
自动构建记录缺失时补投 GitHub webhook（API 失败时最多 3 次）；必须配置在 `production` Environment，并限制为仅对
...
`awaiting_expand_confirmation` 耐久中间态，随后在同一次 Promotion 自动收尾；不提供独立手动入口。自动收尾必须先取得 `/run/lock/agent-saas/promotion.lock`
的 PID/start-time/FD owner proof；锁 holder 的租约上限必须长于自动收尾步骤的 30 分钟上限，并在两次 Production live/API ready 读回、
...
5 分钟 live/evidence 新鲜度；通用 operation key 或陈旧证据都不能绕过。确认窗口一旦过期，自动收尾立即 fail closed，不允许同一次确认自动续期或
追加自迁移；重跑等待中的发布只续做确认，不重新部署组件；GitHub 已 completed 时仅允许同一 run 修复 OSS 镜像。过期需要基于当前 main 和当前生产基线重新创建 RC。任何分叉 attestation、同路径
