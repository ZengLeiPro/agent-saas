...
3. 记录缺失持续 30 秒后，只补投同一 SHA、`refs/heads/main` 且失败的 GitHub push delivery。已成功或已补投的同 GUID 不再补投；不调用 ACR 手动构建 API。GitHub API 失败（含 422）时在 45 分钟窗口内退避再补投，最多 3 次（间隔 120s、300s）。缺配置、找不到可补投 delivery、或三次仍失败，立即 fail closed。
...
无对应失败 delivery、已返回 2xx 却未生成构建、三次补投仍失败、扫描超过安全范围时，必须查看 GitHub delivery 与 ACR 源码绑定/授权状态；自动恢复会明确停止，不盲目重建。超时导致补投结果不确定时，后续运行先读投递记录，不直接重发。
