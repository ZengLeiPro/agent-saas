...
### D-04 复核修正：运行对账与最终验收资格

`staging-final.json` 使用 `runtimeConverged` 表示观测时 API、Worker、Web、ACS 身份及健康检查是否达到目标，不再输出 `acceptanceAllowed`。报告生成之后仍可能发生 SSH 撤权或证据上传失败，最终验收资格以全部收尾步骤之后写入的 GitHub deployment 状态为准。

回归测试执行 Workflow 中实际的 SSH 撤权及最终 deployment 状态脚本：连续五次撤权失败时，运行对账仍可为 true，但最终状态必须为 failure，验收门禁必须拒绝。
