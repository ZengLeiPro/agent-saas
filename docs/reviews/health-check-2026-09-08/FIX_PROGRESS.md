...
| DEP-01 | 修复中   | Mobile Markdown 解析链升级与资源预算回归 | 锁树仅保留安全版本；等待 PR CI 与合并                |
...
### 2026-09-11 DEP-01 解析器升级

- 从当时最新 `origin/main` 的 `cc99440d43cdfcb22795260835614fe53eef8035` 创建独立分支 `fix/mobile-markdown-dep01`，未改动原工作区。
- Mobile 直接依赖升级并精确锁定为 `marked@18.0.12`、`markdown-it@14.3.1`；对 `react-native-markdown-display` 的旧 `markdown-it@^10.0.0` 传递链增加定向 override。最终锁树仅保留 `markdown-it@14.3.1`、`linkify-it@5.0.2` 和 `marked@18.0.12`，相关公告不再被 `pnpm audit` 命中。
- 新增独立受限 Node 子进程回归，覆盖 Marked 三字节 OOM、markdown-it newline ReDoS 和 linkify-it `mailto:` 二次复杂度输入；保留并扩充中文标点、中英文混排、链接、列表、代码块与 typographer 兼容测试。
- 本地 Mobile 定向 9 项、类型检查、Expo 依赖检查、iOS/Android Router export 与 207 项原生/release contracts 均通过。完整 `pnpm -F mobile test` 的 Vitest 阶段通过；随后仍会遇到本文件已记录的 Node 22.21.1 嵌套 pnpm engine warning 污染 JSON stdout，直接运行相同 contracts 全部通过。正式关闭仍等待锁定 Node 22.23.1 的 PR CI。
- 未修改 Workflow、发布配置或业务数据，未执行部署。
