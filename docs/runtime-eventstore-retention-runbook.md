...
合并后至少观察两个 sweep 周期，并覆盖一次 worker 蓝绿/回退交接：确认 never-run 派生、合法启动先出现当前配置的 scheduled、启动门禁 blocked、running/成功状态按时更新、同类后批失败保留累计删除、乱序写入后 lastSuccessAt 单调、容量三分量和 total 对齐、容量趋势从不足 2 个有效样本的“需关注”恢复为“健康”、旧样本不显示健康、stale 在采样恢复后解除；同时检查固定的状态落库告警、system_metrics 增长量、接口 P95 延迟和平台管理员 403 门禁。
