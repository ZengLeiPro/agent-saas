...
- retention 过期阈值为 `max(2 × sweepInterval, 30 分钟)`；容量过期阈值为 30 分钟；最新容量或任一趋势样本明显晚于响应生成时间（允许 5 分钟时钟偏差）时，容量整体降级为不可用，前端 schema 同样拒绝越界响应。有 Store 但无运行快照时显示 `never_run`；Store 缺失、状态持久化不可用、生产 active runtime-worker readyfile/PID 不可验证或快照契约不可识别时显示 `unavailable`；未知数值保持 `null`，不得解释成健康或 0。过期只设置 `stale=true`，不会覆盖最近一次 `failed/blocked/succeeded` 结果；页面在总体标记“已过期”的同时继续展示最近可信结果和稳定脱敏 `errorCategory`。API 按 state 校验时间、非负 duration、水位、max sequence 和固定六类分类摘要；成功状态缺字段、非法或明显晚于采样时间的运行时间、状态 sampledAt 明显晚于响应生成时间（允许 5 分钟时钟偏差）、负数、分类缺失/未知、dry-run 出现删除量，或快照 mode/sweep 与当前配置不一致时均返回 `unavailable`。成功水位必须满足 effective=min(legal,billing)，lag 按 max(max−effective,0) 推导；`maxGlobalSequence` 表示当前存活行最大序号，清理最高序号后可以低于单调 billing/effective 水位，此时合法 lag 为 0。未知 state 先归一为 `unavailable`，不能因采样时间较旧而伪装成合法旧结果。写入用事务级 advisory try-lock 做确定性无等待门禁；连接获取失败同样立即把 worker 状态持久化标为不可用，未获锁则回滚并让 worker 启动降级：不武装 timer、不推进 billing projection、不查询 EventStore、不 DELETE，HTTP 仍可启动。启用 retention 时该不可用状态会并入 runtime-worker admission：承载 singleton retention 的 Scheduler 同时停止领取新 run，readyfile 不会建立或会被撤回；ws-only API 复用 active worker readyfile/PID 存活门禁，因此另一个进程内的默认布尔值不能把旧成功误报为当前权威。专用 runtime-worker 在启动阶段无法建立状态权威时直接启动失败，不形成瘸腿常驻进程，由既有 `Restart=on-failure` 在数据库恢复后重试；兼容单进程 `all` 角色保留 HTTP 启动，但 Scheduler 准入关闭，并每 30 秒仅重试 startup 状态写入，成功后才启动 sweep 与恢复准入。`scheduler-only` 不承载 singleton retention，因此不应用此门禁。运行中暂时失败则由后续 retention 状态写入恢复门禁；释放锁后也可显式重试。成功获锁后按受索引支撑的最大 `id` 读取权威行，仅对不晚于数据库 `clock_timestamp()` 5 分钟的受信 `lastSuccessAt` 做单调合并；明显未来的旧值不参与下一次当前成功写入，因此可自动恢复 available。状态行以 `clock_timestamp()` 更新并清理旧重复行；它不依赖事务开始时间，也不按 JSON 字段排序历史。每个 worker 启动快照携带新的内部 writer claim；execute 的 billing projection 在持有当前 authority 单例行锁期间完成，每个 DELETE 批次也在同一事务中锁定并校验 writerId，因此候选 claim 与破坏性副作用严格串行，claim 成功后旧 writer 的下一批 mutation 和 running/末态迟到写都会被拒绝。回滚重启的旧 worker 以新 claim 重新接管，并按调用时真实 in-flight/末态与当前调度重建快照，不会把已结束 sweep 的旧 `running` 刷成新鲜状态。部署先检查 active release 的 `runtime-worker-execution-fencing-v1` capability marker：支持 fencing 时才允许候选 ready/claim 后原子切换 `active-color` marker 并排空旧 worker；旧 runtime 不支持时必须先 drain 并确认对应 runtime-worker 或 bootstrap `all` 进程完全退出，再启动会 claim 的候选，禁止 base/legacy→HEAD 混合版本 destructive overlap。旧 runtime 完全退出后必须重新核对候选 pidfile、readyfile、systemd MainPID 与存活状态，再向候选 PID 请求一次 non-claim authority refresh，并等同 PID ack 后才启动 Web；该 CAS 写只允许当前 writer 或不带 authority 的 legacy 状态，既覆盖首次上线旧二进制的最后一次写，也防止延迟 refresh 抢回后续 worker 的 authority；retention 未启用时 refresh 是成功 no-op。失败回退时，legacy 预排空路径必须先停止候选，再恢复旧 release 的 runtime service template 与 worker；bootstrap `all` 同样先恢复旧 service template 后才重启。legacy graceful drain 超过 960 秒或触及预留回退预算的 soft deadline 时不得启动候选，应在确认旧 unit 已停止后执行 single-writer 回退，未完成的 durable run 由 lease 恢复。新版路径仅确认 unit 已退出、完成 drain 的旧 worker 才 restart；已发送 drain 但尚未退出时保留候选并失败退出，绝不强制重启中断 durable run。仍存活且具备 capability marker 的旧 worker 通过 `SIGHUP` 原地重新 claim 并恢复 ready。首次 `all`→runtime-worker bootstrap 回退会在候选停止前验证仍服务的 active `all` PID/ready/MainPID，并通过 `SIGHUP` 与同 PID ack 恢复 authority；旧 `all` 不具备该 capability 时保留候选 authority 并失败退出，绝不停止唯一权威 writer。正常长 drain 和失败回退都保持 marker 指向已 ready 的 worker。普通 metrics 裁剪同样按最大 `id` 保留该状态，因此蓝绿进程乱序获锁、停用或长期故障后不会回退成更旧成功时间或 `never_run`。
...
合并后至少观察两个 sweep 周期，并覆盖一次 worker 蓝绿/回退交接：确认 never-run 派生、合法启动先出现当前配置的 scheduled、启动门禁 blocked、running/成功状态按时更新、同类后批失败保留累计删除、乱序写入后 lastSuccessAt 单调、候选 claim 后旧 sweep 不再 projection/DELETE 且 reclaim 不重发旧 running、容量三分量和 total 对齐、容量趋势从不足 2 个有效样本的“需关注”恢复为“健康”、旧样本不显示健康、stale 在采样恢复后解除；同时检查固定的状态落库告警、system_metrics 增长量、接口 P95 延迟和平台管理员 403 门禁。
