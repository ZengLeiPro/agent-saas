...
| 2 | 解包 release 到 `releases/<sha>`，`server/data` 软链到 NAS，`pnpm install --frozen-lockfile --filter server... --filter shared...` | 切流前失败，老色照常服务 |
| 3 | 切流前清理旧 release：保留最近 5 个 + current/previous + 两个 color symlink 的 target（active 色 crash-restart、idle 色 drain 中都还要读自己的代码） | 同上 |
...
| 6 | `systemctl start agent-saas-server@<idle>` + **ready 硬门禁**：等 `/api/healthz/ready` 200，最长 180s | `rollback_idle_and_exit`：stop idle 色 + 还原 current/previous symlink，nginx/active-color 全程未动 |
