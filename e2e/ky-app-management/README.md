...
覆盖上传、发布、新组织无安装白名单的组织安装、DNS 配置指引、一次性领取和清除明文。历史空名单与受限名单兼容、成员/Agent 授权隔离由 `scopeRetirement.pg.test.ts` 和 `managementFlow.pg.test.ts` 的真实 HTTP/PG 测试覆盖。生产登录流程未在此夹具中执行；测试身份通过独立头注入。禁止对外开放此测试服务器。
