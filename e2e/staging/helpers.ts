...
export async function login(page: Page): Promise<string> {
  await page.goto('/');
  await expect(messageInput(page)).toBeVisible({ timeout: 30_000 });
...
export async function apiLogin(): Promise<string> {
  const response = await fetch(`${required('STAGING_API_URL')}/api/auth/login`, {
    method: 'POST',
  ...
  if (!response.ok) throw new Error(`Staging API login returned ${response.status}`);
  const body = (await response.json()) as { token?: unknown };
  if (typeof body.token !== 'string' || !body.token)
    throw new Error('Staging API login did not return a token');
  return body.token;
}

export function messageInput(page: Page): Locator {
  return page.getByRole('textbox', { name: '消息输入' });
}
