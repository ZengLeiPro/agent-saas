...
import { showTextPrompt } from '../../lib/prompt';
...
import { usePendingSharedFiles } from '../../contexts/PendingSharedFilesContext';
...
import { useColors, spacing, radius, fontScale, type ThemeColors } from '../../theme';

export type ChatSessionPresentation = 'stack' | 'pane';

export type ChatSessionScreenProps = {
  /** Route or parent-provided session id (`new` allowed). */
  sessionId?: string | null;
  /** `stack` = full push screen; `pane` = md+ master-detail pane (no back stack). */
  presentation?: ChatSessionPresentation;
  /** Pane mode: clear selection / go back to empty state. */
  onClosePane?: () => void;
  /** Pane mode: navigate to another session without leaving the split. */
  onSessionNavigate?: (sessionId: string) => void;
};

/**
 * 会话详情主体 —— 供 `/chat/[sessionId]` 全屏栈与 md+ master-detail 右栏复用。
 */
export function ChatSessionScreen({
  ...
  // Pane embeds its own chrome (~52); stack uses the native header height.
  const headerHeight = isPane ? 52 : stackHeaderHeight;
  ...
  /** User-resized docked pane width (optional AsyncStorage persist). */
  ...
  }, [rightSlotOpen, hostBudget, breakpointWidth, desiredDockedWidth]);
  useEffect(() => {
    if (!rightSlotOpen) {
  ...
  const agentSwitch = useAgentSwitch({
    ...
    currentSession,
    onLaunchNewSession: useCallback(() => {
      if (isPane && onSessionNavigate) {
        onSessionNavigate('new');
        return;
      }
      router.replace('/chat/new');
    }, [router, isPane, onSessionNavigate]),
  });
  ...
  const handleFork = useCallback(async (message: MessageItem) => {
    ...
    if (isPane && onSessionNavigate) {
      onSessionNavigate(newSessionId);
      return;
    }
  ...
  useEffect(() => {
    if (sessionId === 'new') {
      if (chat.sessionId) {
        // Prefer in-pane navigation; stack uses setParams to avoid a slide transition.
        if (isPane && onSessionNavigate) {
          onSessionNavigate(chat.sessionId);
        } else {
  ...
  }, [sessionId, chat.sessionId, navigation, isPane, onSessionNavigate]); // 依赖故意收窄（react-hooks/exhaustive-deps 在本仓库未启用）
  ...
  const paneHeader = (
    <View
      ...
      testID="chat-pane-header"
    >
      <View style={{ width: 44, alignItems: 'flex-start' }}>
        <BackButton onPress={handleHeaderBack} />
      </View>
      <View style={{ flex: 1, alignItems: 'center' }}>{headerTitleNode}</View>
      <View style={{ minWidth: 44, alignItems: 'flex-end' }}>{headerRightNode}</View>
    </View>
  );
      ...
      {activeUsageCard === 'context' && chat.tokenUsage && (
        <TokenDetailOverlay
          ...
          onOpenChildSession={(childSessionId) => {
            ...
            if (isPane && onSessionNavigate) {
              onSessionNavigate(childSessionId);
            } else {
