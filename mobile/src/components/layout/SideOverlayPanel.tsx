 ...
 * - docked + resizable: drag the left edge (web useResizePanel 25–75% clamp lives in parent)
 */
...
import { RIGHT_PANE_DIVIDER_WIDTH } from '../../lib/layoutProtection';
...
export type SideOverlayPanelProps = {
  ...
  /** When docked, show a drag handle on the left edge. */
  resizable?: boolean;
  /** Positive deltaX = pointer moved right (shrink pane). */
...
export function SideOverlayPanel({
  ...
  const showResize = docked && resizable && !!onResizeDelta;
  const lastXRef = useRef<number | null>(null);

  const panResponder = useMemo(
    () =>
      PanResponder.create({
        onStartShouldSetPanResponder: () => showResize,
        onMoveShouldSetPanResponder: () => showResize,
        onPanResponderGrant: (evt) => {
          lastXRef.current = evt.nativeEvent.pageX;
        },
        onPanResponderMove: (evt) => {
          if (!onResizeDelta || lastXRef.current == null) return;
    ...
    [showResize, onResizeDelta, onResizeEnd],
  );
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        panel: {
          ...
          borderLeftWidth: showResize ? 0 : StyleSheet.hairlineWidth,
        ...
        resizeHairline: {
          width: RIGHT_PANE_DIVIDER_WIDTH,
          height: '100%',
    ...
    [colors, insets.top, width, docked, showResize],
  );
      ...
      <View style={styles.panel}>
        {showResize ? (
          <View
            ...
            accessibilityLabel="调整右侧栏宽度"
            {...panResponder.panHandlers}
          >
            <View style={styles.resizeHairline} />
          </View>
        ) : null}
