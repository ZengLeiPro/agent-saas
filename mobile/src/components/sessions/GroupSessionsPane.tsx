/**
 * Group session list body — phone stack screen + md+ in-pane master drill.
 * variant=`screen` keeps Stack.Screen chrome; `embedded` uses an in-pane header.
 */
...
import type { Swipeable } from '../SwipeableRow';
import { GroupPickerSheet } from './GroupPickerSheet';
import { useSessionGroupActions } from './useSessionGroupActions';
import { useSessionRowActions } from './useSessionRowActions';
import { toSidebarSessions } from '../../lib/sessionListAdapter';
import { useColors, spacing, typography, fontScale, fontWeight } from '../../theme';
...
export type GroupSessionsPaneProps = {
  groupKey: string;
  name?: string;
  /** screen = phone stack route; embedded = md+ master-list drill (no stack push). */
  variant?: 'screen' | 'embedded';
  onBack?: () => void;
  /** md+: select into detail pane instead of router.push. */
  onSelectSession?: (sessionId: string) => void;
};

export function GroupSessionsPane({
  groupKey,
  name,
  variant = 'screen',
  onBack,
  onSelectSession,
}: GroupSessionsPaneProps) {
      ...
      try {
        if (isAdminUser) {
          ...
          for (const p of profiles)
            map[p.username] = { avatar: p.avatar, avatarVersion: p.avatarVersion };
          setAvatarMap(map);
        } else if (authUser?.username) {
          const profile = await fetchAgentProfile(authUser.username);
          setAvatarMap({
            [authUser.username]: { avatar: profile.avatar, avatarVersion: profile.avatarVersion },
          });
        }
      } catch {
        /* ignore */
  ...
  const handleSelectSession = useCallback(
    ...
    [chat.selectSession, router, onSelectSession],
  );
  ...
  const handleBatchRemove = useCallback(() => {
    ...
    Alert.alert('移出分组', `确定要将 ${ids.length} 个会话移出当前分组吗？`, [
      { text: '取消', style: 'cancel' },
      {
        text: '移出',
        style: 'destructive',
  ...
  const renderItem = useCallback(
    ({ item }: { item: ChatSessionIndexItem }) => {
  ...
  const handleGroupMenuSelect = useCallback(
    (actionId: string) => {
      if (!groupKey) return;
      if (actionId === '_rename_group') {
        ...
        Alert.alert('删除分组', '删除分组后，分组内的会话不会被删除。', [
          { text: '取消', style: 'cancel' },
          {
            text: '删除',
            style: 'destructive',
            onPress: () => {
              void groupsHook.deleteGroup(groupKey).then(() => handleBack());
            },
          },
        ]);
      }
    },
    [groupKey, groupsHook, displayName, handleBack],
  );
  ...
  const paneHeader = embedded ? (
    ...
    >
      ...
      <View style={[styles.headerTitle, { flex: 1, minWidth: 0 }]}>
        ...
        {isLoadingFull && <ActivityIndicator size="small" color={colors.mutedForeground} />}
      </View>
      {isSelectMode ? (
  ...
  return (
    <View
      ...
      testID={embedded ? 'group-sessions-pane' : 'group-sessions-screen'}
    >
      {paneHeader}
      {!embedded ? (
        <Stack.Screen
          ...
          refreshControl={
            <RefreshControl
              ...
              onRefresh={() => {
                ...
                void Promise.all([chat.refreshSessions(), fetchFullSessions()]).finally(() =>
