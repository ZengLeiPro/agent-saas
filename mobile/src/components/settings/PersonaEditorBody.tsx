...
import { useColors, spacing, typography, fontScale, monoFamily } from '../../theme';
import { createMarkdownStyles } from '../chat/markdownStyles';
import { createMarkdownRules } from '../chat/markdownRules';
import {
  fetchPersona,
  updatePersona,
  parsePersona,
...
export type PersonaEditorBodyProps = {
  username: string;
  mode?: EditorMode;
  /** md+ settings detail: inline chrome instead of Stack.Screen */
  embedded?: boolean;
  onRequestClose?: () => void;
};
...
export function PersonaEditorBody({
  username,
  mode: modeProp = 'persona',
  embedded = false,
  onRequestClose,
}: PersonaEditorBodyProps) {
  const colors = useColors();
  const insets = useSafeAreaInsets();
  const mode: EditorMode = modeProp === 'memory' ? 'memory' : 'persona';
  const config = MODE_CONFIGS[mode];

  useEffect(() => {
    reportActivity(mode === 'memory' ? 'agent_memory_viewed' : 'agent_persona_viewed', {
      detail: username,
    });
  }, [mode, username]);
  ...
  return (
    <View
      ...
      testID={embedded ? 'persona-editor-pane' : 'persona-editor-screen'}
    >
      {embedded ? (
        <View style={styles.embeddedChrome} testID="persona-editor-embedded-chrome">
          <TouchableOpacity
            onPress={() => {
              if (!isViewMode) {
                handleCloseEdit();
