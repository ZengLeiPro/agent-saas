...
async function localLogin(): Promise<void> {
  const response = await post('/api/local/login', {
    loginId: localLoginId.value,
    password: localPassword.value,
  });
  message.value = response.ok ? '业务系统登录成功，请刷新页面继续。' : '账号或密码不正确。';
  if (response.ok) window.location.assign('/orders');
}
</script>
...
<template>
  <section>
    <h2>业务系统登录</h2>
    <fieldset>
      <legend>独立账号登录</legend>
      <input v-model="localLoginId" placeholder="账号" aria-label="独立业务账号" />
      <input v-model="localPassword" type="password" placeholder="密码" aria-label="独立业务密码" />
      <button type="button" @click="localLogin()">登录</button>
    </fieldset>
    <h3>组织接入故障时的兜底方式</h3>
