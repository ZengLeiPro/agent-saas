...
本目录冻结 WP0 的跨实现输入和期望结果。`positive.json` 包含由两把不同测试密钥签署的 enrollment request、installation grant、client assertion、workload access token、DPoP proof 和 attest；`negative.json` 描述必须 fail closed 的变体。
...
5. 时间相关用例将验证时钟固定为 `2026-09-14T00:00:30Z`。URI 比较使用 HTTPS scheme/host 小写、默认端口消除、空 fragment、保留 path/query 的规范化规则。
...
自检只证明 fixture 未损坏且正向签名可验证，不替代 WP1 的语义 validator 测试。
