...
// The append-only review inventory is intentionally historical and can exceed Node's 1 MiB
// spawnSync default. Keep a bounded explicit limit so Git can return the committed document.
const REVIEW_INVENTORY_MAX_BUFFER = 8 * 1024 * 1024;
...
const document = JSON.parse(
  execFileSync('git', ['show', `${target}:${MIGRATION_REVIEWS_PATH}`], {
    encoding: 'utf8',
    maxBuffer: REVIEW_INVENTORY_MAX_BUFFER,
  }),
);
