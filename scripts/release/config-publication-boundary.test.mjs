...
test('old-instance rollback boundary accepts only the signed current online version', () =>
  fixture(async ({ configPath, snapshot }) => {
    await assert.rejects(
      validatePrivateConfigIdentityReleaseBinding({
        privateSnapshotPath: snapshot,
        releaseId: 'release-a',
        expectedConfigIdentity: OLD,
      }),
      /disagrees/u,
    );
...
test('signed config selection does not weaken exact code-release binding', () =>
  fixture(async ({ configPath, snapshot }) => {
    await assert.rejects(
      validatePrivateConfigIdentityReleaseBinding({
        privateSnapshotPath: snapshot,
        releaseId: 'another-code-release',
        expectedConfigIdentity: ONLINE,
        productionConfigPath: configPath,
      }),
      /release binding/u,
    );
...
test('a different code release still uses its own freshly computed candidate identity', () =>
  fixture(async ({ configPath, snapshot, summary }) => {
...
test('unpublished file changes cannot be accepted by rollback validation', () =>
  fixture(async ({ configPath, snapshot }) => {
