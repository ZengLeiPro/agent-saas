...
function withPublished(fn) {
  ...
  try {
    ...
    const record = writePublication(configPath, {
      ...initial,
      revision: randomUUID(),
      sequence: 2,
      rawRevision: rawRevision(next),
      identity: ONLINE,
      changedPaths: ['models'],
      actor: 'authorized-platform-admin',
    });
...
test('steady-state release evidence accepts signed online changes without changing code release identity', () =>
  withPublished(({ configPath }) => {
    assert.throws(() =>
      validateExpectedConfigIdentityObservers(RELEASE_BASELINE, summary(ONLINE), {
        configIdentityStage: 'steady-state',
      }),
    );
    const authorized = publishedExpected(configPath, 'release-a', RELEASE_BASELINE);
    assert.doesNotThrow(() =>
      validateExpectedConfigIdentityObservers(authorized, summary(ONLINE), {
        configIdentityStage: 'steady-state',
      }),
    );
  }));

test('a new code release uses its independently computed expected identity, not another code version digest', () =>
  withPublished(({ configPath, next }) => {
    const nextCodeIdentity = { schemaVersion: 1, digest: `sha256:${'c'.repeat(64)}` };
    assert.deepEqual(
      publishedExpected(configPath, 'release-b', nextCodeIdentity),
      nextCodeIdentity,
    );
    assert.equal(readFileSync(configPath, 'utf8'), next);
  }));

test('returning to the prior code release selects its signed online identity rather than its stale original baseline', () =>
  withPublished(({ configPath }) => {
    assert.deepEqual(publishedExpected(configPath, 'release-a', RELEASE_BASELINE), ONLINE);
    assert.doesNotThrow(() =>
      validateExpectedConfigIdentityObservers(
        publishedExpected(configPath, 'release-a', RELEASE_BASELINE),
        summary(ONLINE),
        { configIdentityStage: 'steady-state' },
      ),
    );
...
test('all configuration identity transport manifests carry the shared signature verifier', () => {
  for (const path of [
    ...
    assert.match(
      ...
      /scripts\/release\/config-publication\.mjs/u,
      `${path} must transport the verifier`,
    );
  }
});
...
test('build and deployment ship a sealed publication CLI and do not bootstrap in the runtime observer', () => {
  ...
  assert.match(
    readFileSync('scripts/release/deploy-production-release.sh', 'utf8'),
    /config-publication-cli\.js" prepare/u,
  );
  assert.doesNotMatch(
    readFileSync('server/src/app/productionModelPublication.ts', 'utf8'),
    /preparePublicationAuthority/u,
  );
});
