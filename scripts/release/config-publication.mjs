...
export function isOwnerAlive(owner) {
  try {
    return canonical(processIdentity(owner.pid)) === canonical(owner);
  } catch {
    return false;
  }
}
...
function version(value) {
  if (
    ...
    !/^[a-zA-Z0-9-]{1,100}$/u.test(value.revision) ||
    !HEX.test(value.rawRevision ?? '')
  )
...
export function validatePublication(value) {
  if (
    ...
    value.changedPaths.some((p) => typeof p !== 'string') ||
    !Number.isFinite(Date.parse(value.updatedAt ?? ''))
  )
    throw new Error('Invalid configuration publication');
  ...
  if (
    ...
    (!previous ||
      ...
      value.owner.pid < 1 ||
      typeof value.owner.startTicks !== 'string' ||
...
export function readPublication(configPath) {
  ...
  if (
    ...
    !verify(
      null,
      Buffer.from(DOMAIN + canonical(envelope.record)),
...
export function signingAvailable(configPath) {
  try {
    ...
    return (
      createPublicKey(createPrivateKey(readFileSync(privatePath))).export({
        type: 'spki',
        format: 'pem',
...
export function writePublication(configPath, input) {
  ...
  const signature = sign(
    null,
    Buffer.from(DOMAIN + canonical(record)),
...
export function readSnapshot(configPath, digest) {
  ...
  if (rawRevision(text) !== digest)
    throw new Error('Configuration snapshot integrity check failed');
  return text;
}
...
export function publishedExpected(configPath, releaseId, fallback, requireCommitted = true) {
  ...
  if (requireCommitted && record.phase !== 'committed')
    throw new Error('Production configuration transaction requires recovery');
...
export function preparePublicationAuthority(configPath, releaseId, expected) {
  ...
  if (existsSync(root)) {
    ...
    if (!record || record.phase !== 'committed')
      throw new Error('Resolve pending configuration transaction before deployment');
  ...
  try {
    ...
    const record = validatePublication({
      ...
      rawRevision: digest,
      identity: expected,
      actor: 'controlled-code-release',
      changedPaths: [],
      updatedAt: new Date().toISOString(),
    });
    const envelope = `${JSON.stringify({
      record,
      signature: sign(null, Buffer.from(DOMAIN + canonical(record)), pair.privateKey).toString(
        'base64',
      ),
    })}\n`;
    ...
    if (readFileSync(configPath, 'utf8') !== text)
      throw new Error('Configuration changed during authority preparation');
...
export function publicationEventCount(configPath) {
  return readdirSync(join(authorityDirectory(configPath), 'events')).filter((name) =>
    name.endsWith('.json'),
  ).length;
}
