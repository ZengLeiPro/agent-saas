...
function intent(path) {
  ...
  return {
    ...
    previous: {
      ...
      rawRevision: previous.rawRevision,
...
test('legacy absence does not initialize or accept a new authority', () =>
  fixture(({ path }) => {
...
test('controlled initialization is durable, private and does not change config bytes', () =>
  fixture(({ path }) => {
...
test('authorized commit survives fresh reads while immutable release fallback is retained', () =>
  fixture(({ path }) => {
    ...
    const committed = writePublication(path, {
      ...next,
      phase: 'committed',
...
test('a plain file edit is rejected by runtime AND deployment readers', () =>
  fixture(({ path }) => {
    preparePublicationAuthority(path, 'rc-1', IDENTITY);
...
test('tampering with a signed head is never silently adopted', () =>
  fixture(({ path }) => {
    preparePublicationAuthority(path, 'rc-1', IDENTITY);
...
test('pending and recovery_required transactions block code deployment', () =>
  fixture(({ path }) => {
    preparePublicationAuthority(path, 'rc-1', IDENTITY);
    const next = intent(path);
    atomicWrite(path, readSnapshot(path, next.rawRevision));
...
test('interruption before file replacement retains the old recovery snapshot', () =>
  fixture(({ path }) => {
...
test('snapshots enforce content integrity and reject traversal', () =>
  fixture(({ path }) => {
    const state = preparePublicationAuthority(path, 'rc-1', IDENTITY);
    assert.throws(() => readSnapshot(path, '../private.pem'), /Invalid/u);
    writeFileSync(
      join(authorityDirectory(path), 'snapshots', `${state.rawRevision}.jsonc`),
      'tampered',
    );
    assert.throws(() => readSnapshot(path, state.rawRevision), /integrity/u);
  }));

test('unsafe authority permissions and symlink replacement fail closed', () =>
  fixture(({ root, path }) => {
    preparePublicationAuthority(path, 'rc-1', IDENTITY);
...
test('different environment directories never share keys or configuration state', () =>
  fixture(({ root, path }) => {
