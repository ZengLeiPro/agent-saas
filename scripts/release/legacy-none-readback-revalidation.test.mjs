...
import {
  LEGACY_NONE_PRODUCER_BLOB, revalidateLegacyNoneReadback, bindLegacyRevalidation,
...
const digest = (value) => digestBuffer(canonicalJson(value));
const asBytes = (value) => Buffer.from(canonicalJson(value) + '\n');
const parseLegacy = (bytes) => JSON.parse(bytes.toString().replace('"postconditionsDigest":undefined,', ''));

function fixture(now = Date.now()) {
  const repository = 'owner/agent-saas';
  const manifest = {
    releaseId: 'rc-20260911-117', releaseSha: 'a'.repeat(40), digest: `sha256:${'b'.repeat(64)}`,
    promotionPolicy: { expiresAt: iso(now + 3600000) },
    migrationPlan: { phase: 'none', planDigest: `sha256:${'c'.repeat(64)}`,
      confirmation: 'not_required', contract: 'separate_release' },
  };
  const entry = (state, operationKey, at, reason) => ({
    state, operationKey, recordedAt: iso(at), releaseId: manifest.releaseId,
    manifestDigest: manifest.digest, ...(reason ? { reason: JSON.stringify(reason) } : {}),
  });
  const history = [
    entry('built', 'build:701', now - 1200000),
    entry('staging_deployed', 'staging:701:1', now - 601000, {
      stagingDeploymentId: '801', stagingRunId: '701', manifestDigest: manifest.digest,
    }),
    ...
    entry('promoting', 'promoting:901:1', now - 300000, {
      releaseId: manifest.releaseId, releaseSha: manifest.releaseSha,
      manifestDigest: manifest.digest, migrationPhase: 'none',
  ...
  const run = {
    id: 701, run_attempt: 1, head_sha: manifest.releaseSha,
    repository: { full_name: repository }, head_repository: { full_name: repository },
    head_branch: 'main', event: 'workflow_dispatch', path: '.github/workflows/deploy-staging.yml',
    status: 'completed', conclusion: 'success',
    run_started_at: iso(now - 1200000), updated_at: iso(now - 578000),
  };
  const historical = {
    schemaVersion: 1, releaseId: manifest.releaseId, manifestDigest: manifest.digest,
    planDigest: manifest.migrationPlan.planDigest, postconditionsDigest: undefined,
    environment: 'staging', observedAt: iso(now - 610000), status: 'not_required', checks: [],
  };
  const deployment = {
    id: 801, environment: 'staging', sha: manifest.releaseSha,
    payload: { releaseId: manifest.releaseId, manifestDigest: manifest.digest, stagingRunId: '701' },
  };
  const state = (id, value, at) => ({
    id, state: value, environment: 'staging', created_at: iso(at),
    deployment_url: `https://api.github.com/repos/${repository}/deployments/801`, log_url: '',
  });
  return {
    manifest, history, bytes: asBytes(historical), producerBlob: LEGACY_NONE_PRODUCER_BLOB,
    run, runId: '701', runAttempt: '1', repository, now, deployment,
...
const receiptReport = (f, receipt = recover(f)) => ({
  receipt, manifest: f.manifest, history: f.history, repository: f.repository, bytes: f.bytes, now: f.now,
...
test('interrupted NONE promotion adds distinct proof while preserving invalid original bytes', () => {
  const f = fixture(); const before = Buffer.from(f.bytes); const proof = recover(f);
  ...
  assert.deepEqual(proof.verification, {
    kind: 'manifest_none_plan', databaseAccessed: false, stagingRerun: false, status: 'not_required', checks: 0,
  });
...
const mutations = {
  'fresh verified candidate': (f) => { f.history = f.history.slice(0, 3); },
  'before-change retry': (f) => { f.history = f.history.slice(0, 4); f.history.push({ ...f.history.at(-1), state: 'failed_before_change' }); },
  'unreviewed promoting tail': (f) => f.history.pop(),
  'completed transaction': (f) => { f.history.at(-1).state = 'completed'; },
  'rolled back transaction': (f) => { f.history.at(-1).state = 'rolled_back'; },
  'rejected acceptance': (f) => { f.history.at(-1).state = 'rejected'; },
  'foreign history RC': (f) => { f.history[3].releaseId = 'rc-20260911-118'; },
  'foreign history manifest': (f) => { f.history[3].manifestDigest = `sha256:${'1'.repeat(64)}`; },
  'unknown producer': (f) => { f.producerBlob = '0'.repeat(40); },
  'expand plan': (f) => { f.manifest.migrationPlan.phase = 'expand'; },
  'contract plan': (f) => { f.manifest.migrationPlan.contract = 'execute'; },
  'none with postconditions': (f) => { f.manifest.migrationPlan.postconditions = []; },
  'different repository': (f) => { f.repository = 'foreign/repo'; },
  'different run': (f) => { f.run.id = 702; },
  'different attempt': (f) => { f.run.run_attempt = 2; },
  'different source': (f) => { f.run.head_sha = '0'.repeat(40); },
  'different branch': (f) => { f.run.head_branch = 'workbench'; },
  'different workflow': (f) => { f.run.path = '.github/workflows/ci.yml'; },
  'different trigger': (f) => { f.run.event = 'push'; },
  'failed staging': (f) => { f.run.conclusion = 'failure'; },
  'expired RC': (f) => { f.manifest.promotionPolicy.expiresAt = iso(f.now - 1); },
  'missing expiry': (f) => { delete f.manifest.promotionPolicy.expiresAt; },
  'future staging': (f) => { f.run.updated_at = iso(f.now + 120000); },
  'oversize evidence': (f) => { f.bytes = Buffer.alloc(4097); },
  'malformed UTF-8': (f) => { f.bytes = Buffer.from([0xff]); },
  'different invalid JSON': (f) => { f.bytes = Buffer.from('{bad secret-token'); },
  'duplicate key': (f) => { f.bytes = Buffer.from(f.bytes.toString().replace('"schemaVersion":1', '"schemaVersion":1,"schemaVersion":1')); },
  'double undefined defect': (f) => { f.bytes = Buffer.from(f.bytes.toString().replace('"postconditionsDigest":undefined,', '"postconditionsDigest":undefined,"postconditionsDigest":undefined,')); },
  'noncanonical whitespace': (f) => { f.bytes = Buffer.from(' ' + f.bytes.toString()); },
  'valid JSON is not legacy proof': (f) => { f.bytes = asBytes(parseLegacy(f.bytes)); },
};
for (const [name, mutate] of Object.entries(mutations)) {
  test(`legacy recovery rejects ${name}`, () => {
    const f = fixture(); mutate(f); assert.throws(() => recover(f));
  });
}
for (const [field, value] of [
  ['releaseId', 'rc-20260911-118'], ['releaseSha', '0'.repeat(40)],
  ['manifestDigest', `sha256:${'0'.repeat(64)}`], ['migrationPhase', 'expand'],
  ['migrationPlanDigest', `sha256:${'0'.repeat(64)}`],
]) {
  test(`promoting marker must bind the same ${field}`, () => {
    const f = fixture(); const reason = JSON.parse(f.history[4].reason); reason[field] = value;
    f.history[4].reason = JSON.stringify(reason); assert.throws(() => recover(f));
  });
}
for (const [field, value] of [
  ['releaseId', 'rc-20260911-118'], ['manifestDigest', `sha256:${'0'.repeat(64)}`],
  ['planDigest', `sha256:${'0'.repeat(64)}`], ['environment', 'production'],
  ['status', 'passed'], ['schemaVersion', 2], ['checks', [{ id: 'not-empty' }]],
  ['observedAt', iso(0)], ['unknownField', 'private-token'],
]) {
  test(`archived legacy evidence still validates ${field}`, () => {
    const f = fixture(); f.bytes = asBytes({ ...parseLegacy(f.bytes), [field]: value, postconditionsDigest: undefined });
    assert.throws(() => recover(f));
  });
}
for (const name of ['hash', 'history', 'stale', 'report', 'repository', 'attempt', 'original bytes', 'verification semantics']) {
  test(`supplement binding rejects changed ${name}`, () => {
    const f = fixture(); const input = receiptReport(f);
...
async function sandbox(t) {
  ...
  git('init', '-q'); git('config', 'user.name', 'Recovery Test'); git('config', 'user.email', 'recovery@example.invalid');
  await mkdir(join(directory, 'scripts/release'), { recursive: true });
  await writeFile(join(directory, 'scripts/release/read-migration-postconditions.mjs'),
    await readFile(join(root, 'fixtures/legacy-none-readback-producer.source.txt')));
  git('add', 'scripts'); git('commit', '-qm', 'Historical producer fixture');
  f.manifest.releaseSha = git('rev-parse', 'HEAD').trim();
  f.run.head_sha = f.manifest.releaseSha; f.latestRun.head_sha = f.manifest.releaseSha;
  f.deployment.sha = f.manifest.releaseSha;
  const marker = JSON.parse(f.history[4].reason); marker.releaseSha = f.manifest.releaseSha;
  f.history[4].reason = JSON.stringify(marker);
  const evidence = join(directory, 'evidence');
  await mkdir(join(evidence, 'attempt-evidence'), { recursive: true });
  const paths = { manifest: join(directory, 'manifest.json'), history: join(directory, 'history.jsonl'), evidence };
  const save = async () => {
  ...
  await save();
  const invoke = (mode = 'revalidate', extra = []) => spawnSync(process.execPath,
...
test('real CLI: strict reader rejects old bytes; new proof is create-only and binds approval', async (t) => {
  ...
  const strictResult = spawnSync(process.execPath, [strict, s.paths.manifest, original,
    join(s.paths.evidence, 'staging-attempt.json'), '701', '1', s.f.repository], { encoding: 'utf8' });
  ...
  const bound = s.invoke('bind'); assert.equal(bound.status, 0, bound.stderr);
  assert.equal(JSON.parse(await readFile(join(s.paths.evidence, 'report.json')))
    .databaseReadbackRevalidation.originalFormatValid, false);
});
...
test('real CLI fails closed on extra args, symlinks and private malformed content', async (t) => {
  ...
  const original = join(s.paths.evidence, 'attempt-evidence/staging-database-readback.json');
  await rm(original); await symlink(s.paths.manifest, original);
  assert.equal(s.invoke().status, 1);
  await rm(original); await writeFile(original, '{private-token=not-for-logs');
  const result = s.invoke(); assert.equal(result.status, 1);
...
test('real production preflight shell revalidates only interrupted legacy NONE and keeps final checks', async (t) => {
  const s = await sandbox(t); const f = s.f;
  const smokeBody = {
    schemaVersion: 1, status: 'passed', environment: 'staging', releaseId: f.manifest.releaseId,
    sourceSha: f.manifest.releaseSha, manifestDigest: f.manifest.digest, stagingRunId: '701',
    stagingRunAttempt: '1', actor: 'staging-e2e-admin', observedAt: iso(f.now - 600000),
    checks: ['login', 'authenticated-read', 'persistence-read', 'websocket'],
  };
  const source = join(s.directory, 'archive'); await mkdir(source);
  await writeFile(join(source, 'staging-core-smoke.json'), JSON.stringify({ ...smokeBody, evidenceDigest: digest(smokeBody) }));
  await writeFile(join(source, 'staging-database-readback.json'), f.bytes);
  const bin = join(s.directory, 'bin'); await mkdir(bin);
  ...
  const invoke = (out) => spawnSync('bash', [shell, s.paths.manifest, s.paths.history, out], {
    cwd: s.directory, encoding: 'utf8', timeout: 30000,
    env: { ...process.env, PATH: `${bin}:${process.env.PATH}`, GITHUB_REPOSITORY: f.repository },
  });
  const out = join(s.directory, 'preflight');
  const result = invoke(out); assert.equal(result.status, 0, result.stderr + result.stdout);
  const report = JSON.parse(await readFile(join(out, 'report.json')));
  assert.equal(report.status, 'passed'); assert.ok(report.databaseReadbackRevalidation.digest);
  assert.deepEqual(await readFile(join(out, 'attempt-evidence/staging-database-readback.json')), f.bytes);
  // Fresh candidates remain blocked; the old invalid artifact is not generally accepted.
  f.history = f.history.slice(0, 3); await s.save();
  const fresh = invoke(join(s.directory, 'fresh')); assert.notEqual(fresh.status, 0);
  ...
  const normal = invoke(join(s.directory, 'normal')); assert.equal(normal.status, 0, normal.stderr);
  ...
  config.statuses[0][0].state = 'failure'; await writeFile(configPath, JSON.stringify(config));
  const revoked = invoke(join(s.directory, 'revoked')); assert.notEqual(revoked.status, 0);
});

test('revalidated evidence does not turn a revoked or superseded Staging run into success', () => {
  const f = fixture(); recover(f);
