  ...
  if (!identity.components || typeof identity.components !== 'object' || Array.isArray(identity.components)) {
    ...
    for (const component of COMPONENTS) {
      ...
      if (!isFullSha(entry.gitSha)) {
        blockingReasons.push(`Production runtime identity component "${component}" must have a complete gitSha.`);
      }
      if (typeof entry.deployedAt !== 'string' || Number.isNaN(Date.parse(entry.deployedAt))) {
        blockingReasons.push(`Production runtime identity component "${component}" must have an ISO deployedAt timestamp.`);
      }
    }
  }
