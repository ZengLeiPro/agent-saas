...
function fixture() {
  ...
  return {
    workflow,
    initialRun: structuredClone(run),
    finalRun: run,
    pages: [{ total_count: jobs.length, jobs }],
    sha,
    repository,
  };
}
...
for (const field of ['initialRun', 'finalRun']) {
  for (const [property, value] of Object.entries({
    workflow_id: 99,
    path: '.github/workflows/acs-sandbox.yml',
...
test('allows a successful carried job from an earlier attempt returned by filter=latest', () => {
  const input = fixture();
  input.pages[0].jobs[1].run_attempt = 1;
...
for (const conclusion of ['failure', 'cancelled', 'skipped', 'neutral', 'timed_out', null]) {
  for (const index of [0, 1]) {
    test(`rejects gate ${index} with conclusion ${conclusion}`, () => {
      const input = fixture();
      input.pages[0].jobs[index].conclusion = conclusion;
...
for (const mutation of [
  (input) => {
    input.pages[0].jobs.pop();
    input.pages[0].total_count = 1;
  },
  (input) => {
    input.pages[0].jobs[1].name = 'Build & Check';
  },
  (input) => {
    input.pages[0].jobs[1].run_id = 101;
  },
  (input) => {
    input.pages[0].jobs[1].head_sha = 'b'.repeat(40);
  },
  (input) => {
    input.pages[0].jobs[1].run_attempt = 3;
  },
  (input) => {
    input.pages[0].jobs[1].run_attempt = 0;
  },
  (input) => {
    input.pages[0].jobs[1].status = 'queued';
  },
  (input) => {
    input.pages[0].jobs[1].id = 1;
  },
  (input) => {
...
test('reads gates beyond page one and refuses pagination drift', () => {
  const input = fixture();
  const otherJobs = Array.from({ length: 100 }, (_, index) => ({
    id: index + 10,
    name: `test-${index}`,
  }));
  input.pages = [
