...
test('manual production boundaries and stable artifact identity are preserved', () => {
  ...
  assert.match(deploy, /if: github.event_name == 'workflow_dispatch' && github.ref == 'refs\/heads\/main'/u);
  assert.match(ci, /name: release-packages-\$\{\{ github.sha \}\}-\$\{\{ github.run_id \}\}-\$\{\{ github.run_attempt \}\}/u);
...
test('Staging consumes a paginated job list and rereads the same unified run before writing evidence', () => {
  const section = staging.slice(staging.indexOf('- name: 解析同一发布 SHA 的 ACS 证据'),
  ...
  assert(section.indexOf('/actions/runs/$APP_CI_RUN_ID"') < section.indexOf('node scripts/release/unified-ci-evidence.mjs'));
});

test('aggregate executes and rejects failed, cancelled or unexpectedly skipped ACS', () => {
  const aggregate = job(ci, 'build');
  const run = aggregate.slice(aggregate.indexOf('        run: |\n') + '        run: |\n'.length)
    .split('\n').map((line) => line.startsWith('          ') ? line.slice(10) : line).join('\n');
  assert.match(run, /exit "\$failed"/u);
  const env = { ...process.env };
  for (const name of [
    'CI_PLAN', 'PREFLIGHT_CHECKS', 'MIGRATION_REVIEWS', 'TESTS', 'POSTGRES_CONTRACTS',
    'WEB_PRODUCTION', 'MOBILE_ROUTER_EXPORT', 'MOBILE_CONTRACT', 'RELEASE_PACKAGES',
    'WRITER_BUNDLE', 'BROWSER_SMOKE', 'ACS_IMPACT_GATE',
  ...
  for (const status of ['failure', 'cancelled', 'skipped', '']) {
    assert.equal(spawnSync('bash', ['-c', run], {
...
for (const [file, required] of [
  ['README.md', false], ['web/src/App.tsx', false],
  ['acs-orchestrator/src/config.ts', true], ['pnpm-lock.yaml', true],
  ['server/src/dws/authFlow.ts', true], ['server/src/runtime/sandboxLifecycleService.ts', true],
  ['.github/workflows/ci.yml', true], ['scripts/ci-acs-plan.mjs', true],
...
test('publish and contract changes select only one ACS gate; malformed output never skips it', () => {
  for (const [publish, contract, required] of [
    ...
    assert.equal(parseClassification(`publish=${publish}\ncontract_check=${contract}\nreason=test`).required, required);
  }
  for (const output of ['', 'publish=false', 'publish=false\ncontract_check=maybe',
    'publish=garbage\ncontract_check=true', 'publish=false\npublish=true\ncontract_check=false']) {
