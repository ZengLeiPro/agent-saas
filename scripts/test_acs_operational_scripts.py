...
class AcsWorkflowRollbackTest(unittest.TestCase):
    ...
    def test_restores_per_pod_snat_before_replacing_the_running_process(self):
        prepare = self.workflow.index('if prepare_snat_rollback; then')
        replaced = self.workflow.index('PROCESS_REPLACED=true')
        self.assertLess(prepare, replaced)
        self.assertIn('rm -f "$SNAT_OPERATION_STATE_FILE"', self.workflow[prepare:replaced])

    def test_code_rollback_does_not_depend_on_candidate_health(self):
        rollback = self.workflow.split('          rollback() {', 1)[1].split('\n          }', 1)[0]
