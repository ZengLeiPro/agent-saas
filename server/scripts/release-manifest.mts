...
import {
  ...
  validateManifest,
} from '../src/release/releaseManifestStore.js';

function usage(): never {
  throw new Error(
    'Usage: release-manifest.mts <create|validate|digest|diff> <manifest.json> [other-manifest.json]',
  );
}
...
const { digest: suppliedDigest, ...unsignedInput } = input as Record<string, unknown>;
const unsigned = releaseManifestContentSchema.parse(unsignedInput);
const calculated = calculateManifestDigest(unsigned);

if (command === 'digest') {
  process.stdout.write(`${calculated}\n`);
} else {
  const parsed = validateManifest({ ...unsigned, digest: suppliedDigest });
  const { digest } = parsed;
  if (command === 'validate') {
    ...
    const changes = Object.fromEntries(
      Object.keys(parsed.components).map((component) => [
        component,
        {
          before: other.components[component as keyof typeof other.components],
          after: parsed.components[component as keyof typeof parsed.components],
        },
      ]),
    );
    process.stdout.write(
      `${canonicalJson({ releaseId: parsed.releaseId, comparedTo: other.releaseId, changes })}\n`,
    );
  } else {
    usage();
  }
