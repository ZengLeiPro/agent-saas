...
export function createFakeOpenAI({ gateFinalText = false, firstTextTimeoutMs = 10_000 } = {}) {
  let count = 0;
  let closed = false;
  ...
  const handleRequest = async (req, res) => {
    ...
    if (!hasToolOutput) {
      send({ choices: [{ delta: { tool_calls: [{ index: 0, id: `call-${randomUUID()}`, type: 'function', function: { name: 'Shell', arguments: JSON.stringify({ command: 'for i in 1 2 3; do echo MP_E2E_$i; sleep 1; done', timeoutMs: 15_000 }) } }] } }] });
