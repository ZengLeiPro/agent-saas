 ...
 *     以及非图片 MIME 被 multer fileFilter 拒绝 → 路由级捕获 400 JSON（修复后行为）。
...
describe('POST /avatar（本人头像上传）与 GET /avatar/:userId 文件服务', () => {
  ...
  it('非图片 MIME 被 fileFilter 拒绝 → 路由级捕获返回 400 JSON（修复后行为）', async () => {
    // auth.ts 的 avatar 路由与 agents.ts/orgAgents.ts 同范式包裹 multer：
    // fileFilter 抛出的「仅支持 PNG、JPEG、WebP 格式的图片」被捕获为 400 JSON，
    // 不再落到 Express 默认错误处理器变成 500 HTML。
    h.setCaller(h.users.wainUser);
    const res = await h.request('/api/auth/avatar',
      { method: 'POST', body: fileForm({ name: 'evil.txt', type: 'text/plain' }) });
    expect(res.status).toBe(400);
    expect((await res.json() as { error: string }).error).toBe('仅支持 PNG、JPEG、WebP 格式的图片');
