...
import { governanceV19Statements } from '../data/governance-schema/v19Migration.js';
...
describe('Governance schema migration SQL fixtures', () => {
  it('V19 在约束前确定性回填 V18 org_memory 的空名称与状态，且名称不引用正文', () => {
    const legacy = {
      tenantId: 'tenant-a',
      resourceId: 'memory-1',
      resourceName: null,
    ...
    const statements = governanceV19Statements({
  ...
  it('V20 ledger DDL 可在 V19 已标记且旧表已存在时幂等升级，并保留 tenant-scoped 唯一键', async () => {
    const statements = governanceV20Statements({ credentialCommits: 'safe_credential_commits' });
    ...
    const applied = new Set(Array.from({ length: 19 }, (_, index) => index + 1));
    ...
    expect(applied.has(20)).toBe(true);
    ...
    expect(queries.filter(item => item.sql.includes('INSERT INTO safe_governance_schema_versions')))
      .toEqual([expect.objectContaining({ params: [20] })]);
