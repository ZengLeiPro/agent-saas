...
describe('Grok production persistence admission', () => {
  it('rejects enabled Grok without a shared pool before contacting any provider', async () => {
    ...
    await expect(createModelSubscriptionRuntime({
      config, secretVault: new InMemorySecretVault(), egressFetch,
  ...
  it('keeps old configurations working but rejects process-local hot enablement', async () => {
    const config = parseAppConfig({
      agent: { cwd: '/tmp/grok-persistence' }, server: { port: 3200 },
    });
    const egressFetch = vi.fn();
    ...
    config.grokSubscription = { enabled: true, credentialRef: 'fixture-ref', quotaCooldownMinutes: 60 };
    expect(() => runtime.grokCredentialManager.getConfiguration()).toThrow('shared_postgres_runtime_required');
