...
export async function createProductionPublicationRig() {
  ...
  const targets: PublicationTarget[] = (['ws-only', 'runtime-worker'] as const).map((role) => ({
    ...
    receiptPath: join(root, `${role}.applied`),
  }));
  ...
  const nodes = await Promise.all(
    targets.map(async (target) => {
      ...
      const resolvers = createModelResolvers({
        ...
        resolveRuntimeModels: async (models) => {
          const resolved = await resolveModelsConfig(models, vault);
          if (!resolved) throw new Error('models missing');
          return resolved;
        },
  ...
  const observe = async () => {
    ...
    for (const node of nodes) {
      ...
      const observed = publicIdentity(
        await computeObservedConfigIdentity(node.config, vault, processCwd),
      );
      ...
      const receipt: PublicationReceipt = {
        ...
        phase: state.phase,
        rawRevision: applied.digest,
        identity: observed,
        recordedAt: Date.now(),
      };
      atomicWrite(
        node.target.receiptPath,
        `${JSON.stringify(corruptReceipt?.(receipt) ?? receipt)}\n`,
      );
  ...
  const input = (
    patch: (value: typeof raw) => void = (value) => {
      value.models.groups[0].models[0].name = 'Updated';
    },
  ): MutationInput => {
    ...
    return {
      ...
      applyRuntime: () => {
        throw new Error('The partial HTTP runtime recipe must never execute in production');
      },
    };
  };
  ...
  return {
    ...
    blockedPhases,
    startHttp,
    setCorruptReceipt: (value: typeof corruptReceipt) => {
      corruptReceipt = value;
    },
    setBeforeObserve: (value: typeof beforeObserve) => {
      beforeObserve = value;
    },
    setTopology: (value: PublicationTarget[]) => {
      topology = value;
    },
    close: async () => {
      if (server)
