...
import {
  assertPublishedDisk, atomicWrite, canonical, processIdentity,
...
if (!configPath || !processCwd || !receiptPath || !expectedJson || !process.send) throw new Error('Invalid test worker arguments');
...
process.on('message', (value: { id: number; command: string }) => {
  work = work.then(async () => {
    ...
    if (!await resolvers.sharedConfigRefresher.refreshIfChanged(true)) throw new Error('Worker refused candidate');
    const observed = await computeObservedConfigIdentity(config, vault, processCwd);
    const identity = { schemaVersion: 1, digest: observed.digest,
      ...(observed.credentialVersionDigest ? { credentialVersionDigest: observed.credentialVersionDigest } : {}) };
    if (canonical(identity) !== canonical(state.identity)) throw new Error('Worker identity mismatch');
    const rawRevision = resolvers.sharedConfigRefresher.getAppliedStamps().config?.digest;
    if (rawRevision !== state.rawRevision || canonical(assertPublishedDisk(configPath)) !== canonical(state)) {
      throw new Error('Worker application became stale');
    }
    atomicWrite(receiptPath, `${JSON.stringify({ schemaVersion: 1, releaseId: 'cross-process-release',
      role: 'runtime-worker', process: processIdentity(), revision: state.revision, sequence: state.sequence,
      phase: state.phase, rawRevision, identity, recordedAt: Date.now() })}\n`);
    process.send!({ id: value.id, ok: true, model: config.models!.groups[0].models[0].value,
      allowed: view.isExecutionAllowed(), rawRevision });
  }).catch((error: unknown) => {
    process.send!({ id: value.id, ok: false, error: error instanceof Error ? error.message : String(error) });
  });
});
