...
describe('ORG_AGENT_SEED_TEMPLATES · 静态模板结构', () => {
  ...
  it('管理端 API 投影逐条来自种子模板并包含可编辑门禁示例', () => {
    ...
    for (const view of views) {
      expect(view.values.avatarStoredPath).toBe(view.values.avatar);
      expect(view.values.allowedKnowledgeText).toBe('');
      expect(view.values.audienceUserIds).toEqual([]);
      expect(view.values.audienceGroupIds).toEqual([]);
      expect(view.values.runtime).toMatchObject({ schemaVersion: 1, model: { strategy: 'inherit' } });
