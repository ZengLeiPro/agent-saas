...
function rig(input:{tenantId?:string; publish?:ReturnType<typeof vi.fn>; getResource?:ReturnType<typeof vi.fn>; select?:ReturnType<typeof vi.fn>; selectionError?:Error}={}) {
  ...
  const upload=createPersonalSkillGovernanceUpload({skills:{getResource,createAndPublishResource:publish} as never,skillConfigStore:{getPoolVisibility:()=>({}),setUserSkillSelected:select} as never,userStore:{findById:(id:string)=>id===actor.id?actor:undefined} as never,agentCwd:join(root,'agents'),sharedDir:join(root,'shared'),tenantSkillsRootDir:join(root,'tenants')});
  return {upload,publish,select,getResource,dir:(id:string)=>join(root,'agents','tenant-a','user-1','.ky-agent','skills',id)};
}
