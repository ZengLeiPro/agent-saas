...
describe('个人 Skill 治理上传服务',()=>{
  ...
  it('zip 目录条目不计入 300 个文件上限',async()=>{const x=rig(); const archive=zip([...Array.from({length:300},(_,index)=>({name:`folder-${index}/`,content:''})),{name:'SKILL.md',content:md('directory-heavy')}]); const r=await x.upload({tenantId:'tenant-a',actorUserId:'user-1',files:[uploadBuffer(archive,'directory-heavy.zip')]}); expect(r).toMatchObject({status:'succeeded',skill:{id:'directory-heavy'}});});
  it.each([
