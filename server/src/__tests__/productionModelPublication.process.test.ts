...
beforeAll(() => {
  ...
  buildSync({
    entryPoints: [resolve('src/__tests__/helpers/productionPublicationWorker.ts')],
    outfile: workerEntry,
    bundle: true,
    platform: 'node',
    format: 'esm',
...
describe('production save across OS processes', () => {
  ...
  const startWorker = async () => {
    const worker = fork(
      workerEntry,
      [configPath, processCwd, join(root, 'worker.applied'), JSON.stringify(expected)],
    ...
    await new Promise<void>((done, reject) => {
      const timeout = setTimeout(
        () => reject(new Error(`Worker did not start: ${diagnostics}`)),
        5_000,
      );
      ...
      worker.once('exit', (code) => {
        clearTimeout(timeout);
        reject(new Error(`Worker exited ${code}: ${diagnostics}`));
      });
    });
    child = worker;
  };
  ...
  beforeEach(async () => {
        ...
        groups: [
          {
            id: 'main',
            name: 'Main',
            baseUrl: 'https://synthetic.example.invalid/v1',
    ...
    expected = {
      ...
      ...(original.credentialVersionDigest
        ? { credentialVersionDigest: original.credentialVersionDigest }
        : {}),
    };
    preparePublicationAuthority(configPath, 'cross-process-release', expected);
    await startWorker();
    queryWorker = () =>
      new Promise<WorkerReply>((done, reject) => {
        const target = child!;
    ...
    observe = async () => {
      ...
      atomicWrite(
        ...
        `${JSON.stringify({
          ...
          phase: state.phase,
          rawRevision: resolvers.sharedConfigRefresher.getAppliedStamps().config?.digest,
          identity: {
            schemaVersion: 1,
    ...
    service = new AdminConfigMutationService({
      configPath,
      processCwd,
      environment: 'production',
      processRole: 'ws-only',
  ...
  const save = (model: string) => {
    ...
    return service.mutate({
      ...
      productionConfirmation: revision,
      buildCandidate: (text) => {
      ...
      applyRuntime: () => {
        throw new Error('Partial update is forbidden');
      },
    });
  };
