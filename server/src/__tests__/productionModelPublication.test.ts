...
describe('production model configuration publication', () => {
  ...
  it('checks the actual candidate scope rather than trusting changedPaths', async () => {
    await expect(
      rig.service.mutate(
        rig.input((raw) => {
          raw.agent.cwd = '/different';
        }),
      ),
    ).rejects.toThrow('其他配置段');
  ...
  it('rejects unresolved auxiliary model references without publishing', async () => {
    await expect(
      rig.service.mutate(
        rig.input((raw) => {
          raw.titleGenerator.model = 'missing/model';
        }),
      ),
    ).rejects.toThrow();
    expect(readPublication(rig.configPath)).toEqual(rig.baseline);
  ...
  it.each([
    ...
    rig.setCorruptReceipt((receipt) => {
      ...
      if (fault === 'wrong-pid')
        return { ...receipt, process: { ...receipt.process, pid: receipt.process.pid + 1 } };
  ...
  it('keeps a durable committed version when only its final acknowledgement is lost', async () => {
    rig.setCorruptReceipt((receipt) =>
      receipt.phase === 'committed' && receipt.revision !== rig.baseline.revision
    ...
    expect(readPublication(rig.configPath)?.phase).toBe('committed');
    expect(JSON.parse(readFileSync(rig.configPath, 'utf8')).models.groups[0].models[0].name).toBe(
      'Updated',
    );
  });
  ...
  it.each([false, true])(
    'recovers a dead publisher with candidate file written=%s',
    async (writeCandidate) => {
      const candidate = JSON.stringify({
        ...rig.raw,
        models: { ...rig.raw.models, allowCrossGroupSwitch: false },
      });
      const candidateIdentity = publicIdentity(
        await computeObservedConfigIdentity(
          parseAppConfig(JSON.parse(candidate)),
          rig.vault,
          rig.processCwd,
        ),
      );
      saveSnapshot(rig.configPath, candidate);
      writePublication(rig.configPath, {
        ...rig.baseline,
        phase: 'applying',
        sequence: rig.baseline.sequence + 1,
        revision: randomUUID(),
        rawRevision: rawRevision(candidate),
  ...
  it('does not overwrite an out-of-transaction disk edit while recovering', async () => {
    ...
    await expect(rig.service.recoverProductionPublication()).rejects.toBeInstanceOf(
      RuntimeRestoreFailedError,
    );
  ...
  it('cannot make two concurrent admin saves overwrite one another', async () => {
    ...
    const second = rig.input((raw) => {
      raw.models.groups[0].models[0].name = 'Other';
    });
    const outcomes = await Promise.allSettled([
      rig.service.mutate(first),
      rig.service.mutate(second),
    ]);
  ...
  it('returns HTTP 200 only after production key and model changes apply to both runtimes', async () => {
    ...
    const body = {
      ...
      models: {
        ...initial.models,
        groups: initial.models.groups.map((group: Record<string, unknown>) => ({
          ...group,
      ...
      productionConfirmation: initial.revision,
    };
    ...
    for (const node of rig.nodes) {
      expect(node.resolvers.modelResolver?.('main/model')?.connection?.apiKey).toBe(
        'replacement-secret',
      );
    ...
    expect(
      await rig.vault.getSecret(rig.oldRef.id, {
        actor: 'system',
        userId: '__system__',
