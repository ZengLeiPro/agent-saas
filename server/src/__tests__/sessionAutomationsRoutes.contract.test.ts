...
async function setup(ownerUserId: string = user.sub) {
  const store = {
    ...
    getCommandReceipt: vi.fn(async () => ({clientMessageId:'message-1',sessionId,commandDigest:'digest',canonicalRequest:{},state:'committed',automationId,response:{result:'created',snapshot},cursor:'1'})),
  ...
  app.use('/api', createSessionAutomationsRouter({ store: store as never, service: service as never, sessionCatalog: sessionCatalog as never, createSession, compensateSession }));
  ...
  return { baseUrl: `http://127.0.0.1:${port}/api`, store, service, createSession, compensateSession };
}
...
describe('Session automation HTTP contract and creation recovery', () => {
  ...
  it('fails closed for attachments and scopes command receipts to the authenticated owner', async () => {
    const rig=await setup();
    const rejected=await fetch(`${rig.baseUrl}/session-automations/commands`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({clientMsgId:'with-file',rawCommand:'/goal -- done',attachments:[{attachmentId:'a'}]})});
