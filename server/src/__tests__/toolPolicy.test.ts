...
describe('DefaultToolPolicy 低风险常开档（TASK-256）', () => {
  ...
  it('safe 工具任何档位都放行', async () => {
    ...
    expect(await decide({ risk: 'safe' }, { autoApproveTools: true, lowRiskOnly: true })).toEqual({
      type: 'allow',
    });
  });
  ...
  it('低风险常开档自动批准 workspace_write，dangerous 仍需人工批准', async () => {
    ...
    await expect(decide({ risk: 'dangerous' }, lowRisk)).resolves.toMatchObject({
      type: 'requires_approval',
    });
  });

  it('全部授权档维持既有行为：非 safe 且非 neverAutoApprove 一律放行', async () => {
    expect(await decide({ risk: 'workspace_write' }, { autoApproveTools: true })).toEqual({
      type: 'allow',
    });
    expect(await decide({ risk: 'dangerous' }, { autoApproveTools: true })).toEqual({
      type: 'allow',
    });
  });

  it('未开启自动批准时非 safe 工具一律人工审批（现状回归）', async () => {
    await expect(decide({ risk: 'workspace_write' }, undefined)).resolves.toMatchObject({
      type: 'requires_approval',
    });
    await expect(decide({ risk: 'dangerous' }, undefined)).resolves.toMatchObject({
      type: 'requires_approval',
    });
  });
  ...
  it('neverAutoApprove 恒为人工，不受任何自动批准档影响', async () => {
    ...
    await expect(
      policy.decide(neverTool, {}, context({ approvalPolicy: { autoApproveTools: true } })),
    ).resolves.toMatchObject({ type: 'requires_approval' });
    await expect(
      policy.decide(
        neverTool,
        {},
        context({ approvalPolicy: { autoApproveTools: true, lowRiskOnly: true } }),
      ),
    ).resolves.toMatchObject({ type: 'requires_approval' });
  });
  ...
  it('resolveCallPolicy 动态降档：dangerous 工具的 safe 调用在低风险档也放行', async () => {
    ...
    expect(await policy.decide(dynamicTool, {}, context({ approvalPolicy: undefined }))).toEqual({
      type: 'allow',
    });
  });
});
...
describe('DefaultToolPolicy 组织 Agent 群聊门禁', () => {
  ...
  it('denies write tools when the current actor role is outside approvalRoles', async () => {
    ...
    const result = await policy.decide(
      descriptor({ id: 'WriteTool', name: 'WriteTool', risk: 'workspace_write' }),
      {},
      {
        channelContext,
    ...
    expect(result).toEqual({
      type: 'deny',
      reason: 'actor role cannot approve this ChannelBinding capability',
    });
  });
  ...
  it('still allows a safe tool for the same actor role', async () => {
    const policy = new DefaultToolPolicy(livePolicy);
    const result = await policy.decide(
      descriptor({ id: 'WriteTool', name: 'WriteTool', risk: 'safe' }),
      {},
      {
        channelContext,
      } as RunContext,
  ...
  it('DwsBusiness 使用群动作矩阵放行共享读并要求低风险写走持久化人工审批', async () => {
    const dwsChannel = {
      channel: 'dingtalk',
    ...
    await expect(
      policy.decide(
        ...
        {
          args: ['doc', 'read', '--node', 'doc-1'],
          credentialMode: 'agent',
        },
        { channelContext: dwsChannel } as RunContext,
      ),
