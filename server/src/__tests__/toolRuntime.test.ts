...
describe('PlatformToolRuntime', () => {
  ...
  it('exposes the merged Artifact tool only when artifact service is configured', () => {
    ...
    expect(ids).not.toContain('CreateArtifact');
  });

  it('keeps a legacy CreateArtifact disablement effective after the Artifact merge', () => {
    const runtime = new PlatformToolRuntime({
      artifactService: {} as never,
      toolControls: { tools: { CreateArtifact: { enabled: false } } },
    });
    expect(runtime.list().map((tool) => tool.id)).not.toContain('Artifact');
  });
