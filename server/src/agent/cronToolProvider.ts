...
const cronManageSchema = z.object({
  ...
  order: z.enum(['asc', 'desc']).optional()
    .describe('comment.list 分页方向，默认 asc（时间升序）。'),
  ...
  ordinal: z.number().int().min(-500).max(500).optional()
    .describe('comment.get 定位：1-based 位置，负数表示倒数第几条（-1=最后一条，默认值）。评论被删除后序号会漂移，跨轮引用请改用 commentId。'),
  commentId: z.string().min(1).max(128).optional()
    .describe('comment.get 定位：评论 id，跨轮稳定。'),
