...
- `display`：只使用无交互的 `callout` / `records` 白名单块。任何带标题的分组、两项及以上判定、清单、对照结果、提交记录都必须使用 `records`；每个 `records.title` 都必填，标题写简短业务动作或结果名称（如「核对工作树状态」「已创建提交」），不得使用空泛的「详情」「结果」。判定清单使用 `layout:"checklist"`，并用 `item.tone` 表示状态：`success`=通过、`danger`=失败、`warn`=警告、`muted`=待定；
