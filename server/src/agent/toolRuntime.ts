...
export class ServerLocalExecutionProvider implements ExecutionProvider {
  ...
  private async _readFile(
    ...
    const target = resolveWorkspaceReadTarget(workspace, path);
    assertSandboxReadAllowed(workspace, target.fullPath);
    return await readWorkspaceFile(target.root, target.fullPath, {
      ...options,
      ...(target.sharedReadOnly ? { displayPath: target.fullPath } : {}),
    }, (fullPath) => {
