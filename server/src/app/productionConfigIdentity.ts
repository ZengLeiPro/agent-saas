...
import {
  assertPublishedDisk, authorityDirectory, publishedExpected, readPublication,
} from '../../../scripts/release/config-publication.mjs';
import {
  computeObservedConfigIdentity, evaluateConfigIdentityStatus, type ExpectedConfigIdentity,
...
export function createProductionConfigIdentityView(options: {
  ...
  if (enabled && existsSync(authorityDirectory(options.configPath)) && !options.expected) {
    throw new Error('Production online configuration requires an independently bound release identity');
  }
  ...
  const mapSummary = (summary: ConfigIdentitySummary): ConfigIdentitySummary => {
    ...
    try {
      ...
      const expected = publishedExpected(options.configPath, options.releaseId, options.expected, false);
      ...
      const observation = summary.observed ? {
        ...summary.observed, credentialVersionDigest: summary.observed.credentialVersionDigest ?? null,
        unresolvedRefPaths: [], computedAt: summary.lastObservedAt ?? new Date(0).toISOString(),
      } : undefined;
      const evaluation = evaluateConfigIdentityStatus(expected, observation);
      const { reason: _reason, ...rest } = mapped;
      return { ...rest, status: evaluation.status, ...(evaluation.reason ? { reason: evaluation.reason } : {}) };
    } catch {
  ...
  return {
    ...
    validatePublishedReload: async (next: AppConfig): Promise<void> => {
      ...
      if (!state) return;
      const expected = publishedExpected(options.configPath, options.releaseId, options.expected, false);
      const observed = await computeObservedConfigIdentity(next, options.secretVault, options.processCwd);
