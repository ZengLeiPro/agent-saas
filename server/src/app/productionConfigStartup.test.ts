...
import {
  createProductionPublicationRig, publicIdentity, type ProductionPublicationRig,
...
import { alignProductionConfigStartup, stageInterruptedProductionConfiguration } from './productionConfigStartup.js';

let rig: ProductionPublicationRig;
beforeEach(async () => { rig = await createProductionPublicationRig(); });
afterEach(async () => { vi.restoreAllMocks(); await rig.close(); });

function stage() {
  return stageInterruptedProductionConfiguration({
    configPath: rig.configPath, processCwd: rig.processCwd,
    releaseId: 'test-release', promotionLockPath: join(rig.root, 'promotion.lock'),
  });
}
async function pending(afterReplacement: boolean, alive = false) {
  ...
  const identity = publicIdentity(await computeObservedConfigIdentity(parseAppConfig(raw), rig.vault, rig.processCwd));
  saveSnapshot(rig.configPath, text);
  const state = writePublication(rig.configPath, {
    ...rig.baseline, revision: randomUUID(), sequence: 2, phase: 'applying',
    rawRevision: rawRevision(text), identity,
...
function productionEnvironment() {
  ...
  vi.stubEnv('AGENT_SAAS_CONFIG_IDENTITY_CREDENTIAL_VERSION_DIGEST', rig.expected.credentialVersionDigest ?? '');
...
async function alignedRuntime() {
  ...
  const identity = await initializeRuntimeConfigIdentityAssembly({
    config, secretVault: rig.vault, processCwd: rig.processCwd,
    logger: { info: vi.fn(), warn: vi.fn() },
  });
  const { sharedConfigRefresher } = createModelResolvers({
    config, processCwd: rig.processCwd, recoveryGate: identity.recoveryGate,
    titleGeneratorConfigs: [], onGuardrailModelConfigsUpdated: () => {},
    getGuardrailModelConfigs: () => [], prepareSystemPromptOverridesUpdate: () => () => {},
...
describe('production cold-start recovery', () => {
  it.each([false, true])('restores the old signed snapshot after replacement=%s without granting readiness', async (afterReplacement) => {
    ...
    expect(rig.appliedPhases.some((value) => value.startsWith('runtime-worker:committed:'))).toBe(true);
  });
  ...
  it('can retry a crash after old bytes were restored but before the pending head was rewritten', async () => {
    ...
    const fault = vi.spyOn(publications, 'writePublication').mockImplementationOnce(() => { throw new Error('injected head interruption'); });
  ...
  it('does not restore a snapshot whose bytes were changed', async () => {
    ...
    writeFileSync(join(rig.root, 'config-publications', 'snapshots', `${rig.baseline.rawRevision}.jsonc`), 'tampered');
  ...
  it('refuses a different code release and preserves the pending journal', async () => {
    const intent = await pending(true);
    await expect(stageInterruptedProductionConfiguration({
      configPath: rig.configPath, processCwd: rig.processCwd, releaseId: 'other-release',
  ...
  it('does not mask failed application, stale applied bytes, dirty runtime or committed denial', async () => {
    ...
    const stamp = vi.spyOn(boot.refresher, 'getAppliedStamps').mockReturnValue({});
    await expect(alignProductionConfigStartup(boot)).rejects.toThrow('启动对齐失败');
    stamp.mockRestore();
  ...
  it('production assembly stages disk recovery before parsing and does not conflate startup with admission', () => {
    ...
    expect(source).toContain('await alignProductionConfigStartup({ processCwd, refresher: sharedConfigRefresher, identity: configIdentityAssembly })');
