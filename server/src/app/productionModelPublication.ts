...
import {
  ...
  publishedExpected, readPublication,
...
import { computeObservedConfigIdentity, evaluateConfigIdentityStatus } from '../release/configIdentity.js';
...
function activeTargets(configPath: string): PublicationTarget[] {
  ...
  return (['ws-only', 'runtime-worker'] as const).map((role) => {
    ...
    const color = readFileSync(join(root, worker ? 'runtime-worker-active-color' : 'active-color'), 'utf8').trim();
    ...
    const mainPid = Number(execFileSync('systemctl', ['show', `${prefix}@${color}.service`, '--property=MainPID', '--value'],
      { encoding: 'utf8', timeout: 2_000 }).trim());
    if (!Number.isSafeInteger(pid) || pid <= 0 || pid !== mainPid) throw new Error('配置目标进程与 systemd MainPID 不一致');
    return { role, process: processIdentity(pid), receiptPath: `/run/${prefix}-${color}.config-identity.json.applied` };
  });
}
...
export function initializeProductionModelPublication(options: {
  ...
  if (runtime.environment !== 'production' || !runtime.releaseId || !runtime.expectedConfigIdentity
    || !snapshotPath || !['ws-only', 'runtime-worker'].includes(options.processRole)) return undefined;
  ...
  const removeReceipt = () => { try { rmSync(receiptPath, { force: true }); } catch { /* Absence fails closed. */ } };
  const observe = (): Promise<void> => {
    if (pending) return pending;
    pending = (async () => {
      if (stopped || (process.env.AGENT_SAAS_DRAIN_MARKER && existsSync(process.env.AGENT_SAAS_DRAIN_MARKER))) {
        removeReceipt(); return;
      }
      const before = assertPublishedDisk(configPath);
      if (!before) { removeReceipt(); return; }
      ...
      if (!await options.refresher.refreshIfChanged(true)
        || options.refresher.getAppliedStamps().config?.digest !== before.rawRevision) {
        removeReceipt(); return;
      }
      const observed = await computeObservedConfigIdentity(options.config, options.secretVault, options.processCwd);
      const selected = publishedExpected(configPath, releaseId, expected, false);
      if (evaluateConfigIdentityStatus(selected, observed).status !== 'consistent') { removeReceipt(); return; }
      await options.identity.refreshSummary();
      const after = assertPublishedDisk(configPath);
      if (stopped || canonical(after) !== canonical(before)
        || options.refresher.getAppliedStamps().config?.digest !== before.rawRevision) {
        removeReceipt(); return;
      }
      const receipt: PublicationReceipt = { schemaVersion: 1, releaseId, role, process: self,
        revision: before.revision, sequence: before.sequence, phase: before.phase,
      ...
      if (Date.now() - lastWarning >= 5_000) {
        ...
        options.logger.warn(`Production config readback unavailable: ${error instanceof Error ? error.message : String(error)}`);
      }
    }).finally(() => { pending = undefined; });
    return pending;
  };
  const publisher = role === 'ws-only' ? new ProductionModelPublisher({
    configPath, processCwd: options.processCwd, releaseId, expected, secretVault: options.secretVault,
    targets: () => activeTargets(configPath), observeLocal: observe,
  }) : undefined;
  const mutationService = publisher ? new AdminConfigMutationService({
    configPath, processCwd: options.processCwd, environment: 'production', processRole: role,
  ...
  const tick = async () => {
    ...
    try {
      ...
      if (!state || state.phase === 'committed'
        || (state.phase !== 'recovery_required' && state.owner && isOwnerAlive(state.owner))) return;
      if (!activeTargets(configPath).some((target) => target.role === role && target.process.pid === process.pid)) return;
      recovering = true;
      await mutationService.recoverProductionPublication();
    } catch { /* Lock held by deploy/save or incomplete peers: keep the signed gate closed and retry. */ }
    finally { recovering = false; }
  };
  const timer = setInterval(() => { void tick(); }, 500);
  timer.unref();
  void tick();
  return { mutationService, observe, stop: () => { stopped = true; clearInterval(timer); removeReceipt(); } };
}
