    ...
    switch (input.event.type) {
      ...
      case 'error':
        emitSession({
          ...
          runId: input.runId,
          client_msg_id: input.clientMsgId,
          error: input.event.error, ...(input.event.failureKind ? { failureKind: input.event.failureKind } : {}), ...(input.event.recoveryAction ? { recoveryAction: input.event.recoveryAction } : {}), ...(input.event.quotaResetAt ? { quotaResetAt: input.event.quotaResetAt } : {}),
        });
        ...
        if (input.userId) {
          ...
          this.eventBus.emitUser(input.userId, {
            ...
            streamId,
            runId: input.runId,
            reason: input.event.error, ...(input.event.failureKind ? { failureKind: input.event.failureKind } : {}), ...(input.event.recoveryAction ? { recoveryAction: input.event.recoveryAction } : {}), ...(input.event.quotaResetAt ? { quotaResetAt: input.event.quotaResetAt } : {}),
          });
  ...
  private async tryResumePersistedInteraction(
    ...
    if (pendingAskUser) {
      ...
      if (TERMINAL_RUN_STATUSES.has(currentRun.status)) {
        ...
        this.sendRespond(client, interactionId, clientAttemptId, undefined, terminalResume.canonicalResponse); if (client.user?.sub && this.eventBus) this.eventBus.emitUser(client.user.sub, { type: 'interaction_resolved', sessionId, interactionId, response: terminalResume.canonicalResponse }, client.ws); return true;
      }
