...
export const DWS_AUTH_SESSION_TTL_MS = 15 * 60 * 1_000;

export type DwsAuthSessionStatus = 'starting' | 'awaiting_user' | 'connected' | 'failed' | 'expired';

export interface DwsAuthSessionIdentity {
  tenantId: string;
  userId: string;
  username: string;
}

export interface DwsAuthSessionRecord extends DwsAuthSessionIdentity {
  sessionId: string;
  status: DwsAuthSessionStatus;
...
export interface DwsAuthSessionStore {
  createOrReuse(identity: DwsAuthSessionIdentity, now?: Date): Promise<{ record: DwsAuthSessionRecord; created: boolean }>;
  markAwaitingUser(sessionId: string, identity: DwsAuthSessionIdentity, userCode: string, authorizationUrl: string, now?: Date): Promise<void>;
  markConnected(sessionId: string, identity: DwsAuthSessionIdentity, now?: Date): Promise<void>;
  markFailed(sessionId: string, identity: DwsAuthSessionIdentity, errorCode: string, errorMessage: string, now?: Date): Promise<void>;
  getLatestForUser(tenantId: string, userId: string): Promise<DwsAuthSessionRecord | null>;
}
...
export class PgDwsAuthSessionStore implements DwsAuthSessionStore {
    ...
    try {
      ...
      await client.query(`
        ...
        SET status = 'expired', error_code = 'authorization_expired',
            error_message = '授权码已过期，请重新连接', completed_at = $3, updated_at = $3
      ...
      const expiresAt = new Date(now.getTime() + DWS_AUTH_SESSION_TTL_MS).toISOString();
  ...
  async markAwaitingUser(
    ...
    await this.options.pool.query(`
      UPDATE ${this.table}
      SET status = 'awaiting_user', user_code = $4, authorization_url = $5, updated_at = $6
      WHERE session_id = $1 AND tenant_id = $2 AND user_id = $3 AND status = 'starting'
    `, [sessionId, identity.tenantId, identity.userId, userCode, authorizationUrl, now.toISOString()]);
  }

  async markConnected(sessionId: string, identity: DwsAuthSessionIdentity, now = new Date()): Promise<void> {
    await this.options.pool.query(`
      UPDATE ${this.table}
      SET status = 'connected', error_code = NULL, error_message = NULL,
          completed_at = $4, updated_at = $4
      WHERE session_id = $1 AND tenant_id = $2 AND user_id = $3
        AND status IN ('starting', 'awaiting_user')
  ...
  async markFailed(
    ...
    const status: DwsAuthSessionStatus = errorCode === 'authorization_expired' ? 'expired' : 'failed';
    await this.options.pool.query(`
      UPDATE ${this.table}
      ...
      WHERE session_id = $1 AND tenant_id = $2 AND user_id = $3
        AND status IN ('starting', 'awaiting_user')
    `, [sessionId, identity.tenantId, identity.userId, status, errorCode, errorMessage.slice(0, 500), now.toISOString()]);
  }

  async getLatestForUser(tenantId: string, userId: string): Promise<DwsAuthSessionRecord | null> {
...
function mapRow(row: Record<string, unknown>): DwsAuthSessionRecord {
  return {
    sessionId: String(row.session_id),
    tenantId: String(row.tenant_id),
    userId: String(row.user_id),
    username: String(row.username),
    status: String(row.status) as DwsAuthSessionStatus,
    ...(stringValue(row.authorization_url) ? { authorizationUrl: stringValue(row.authorization_url) } : {}),
    ...(stringValue(row.user_code) ? { userCode: stringValue(row.user_code) } : {}),
