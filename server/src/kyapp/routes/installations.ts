...
export function createKyAppInstallationsRouter(options: KyAppInstallationRoutesOptions): Router {
  ...
  router.get('/installations', async (req, res) => {
    try {
      const tenantId = managementTenant(
        req.user,
      ...
      res.json(
        await options.management.installations(
          { ...query.data, ...(tenantId ? { tenantId } : {}) },
          req.user!,
        ),
      );
    } catch (error) {
  ...
  router.post('/installations', async (req, res) => {
    if (!req.user || (!isPlatformAdmin(req.user) && req.user.role !== 'admin'))
      return sendKyAppError(req, res, 'forbidden', '需要管理员权限');
  ...
  router.post('/installations/:iid/credentials', requirePlatformAdmin, async (req, res) => {
    ...
    try {
      ...
      const intent = await recordGovernanceIntent(options.audit, governanceActorOf(req.user!), {
        ...
        purpose: 'service_access_rotation',
        metadata: {},
      });
      let ticket;
      try {
        ...
        await recordGovernanceOutcome(options.audit!, intent, 'succeeded', {
          metadata: { credentialId: ticket.credentialId },
        });
      } catch (error) {
    ...
    try {
      ...
      res.json({
        installation: {
          ...
          authMode: installation.authMode ?? 'v1_symmetric',
    ...
    try {
      ...
      res.json({
        ...
        upgrade: {
          ...
          publishedDigest: definition?.publishedDigest ?? null,
          observedDigest: summary?.observedDigest ?? null,
          canSwitch:
            isPlatformAdmin(req.user) &&
            installation.status !== 'deleted' &&
            Boolean(
              summary?.ready &&
              summary.observedDigest === definition?.publishedDigest &&
              summary.observedDigest !== installation.registeredDigest,
            ),
        },
