...
function rowToInstallation(row: Row): KyAppInstallation {
  return {
    ...
    authMode: row.auth_mode === 'v2_asymmetric' ? 'v2_asymmetric' : ('v1_symmetric' as const),
    deploymentId: row.deployment_id == null ? null : String(row.deployment_id),
    currentKeyId: row.current_key_id == null ? null : String(row.current_key_id),
    identityGeneration: Number(row.identity_generation ?? 0),
    createdAt: iso(row.created_at),
    createdBy: String(row.created_by),
    updatedAt: iso(row.updated_at),
    updatedBy: String(row.updated_by),
