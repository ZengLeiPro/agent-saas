...
describePg('套餐到期持久化与审计', () => {
  ...
  it('发布回读 SQL 能验证新增表结构', async () => {
    ...
    const entry = catalog.entries.find((item: { path: string; id?: string }) => item.path === 'server/src/quota/providerQuotaSnapshotStore.ts' && item.id === 'provider-plan-note-schema');
