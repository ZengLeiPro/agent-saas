...
if (command === 'prepare' && configPath && releaseId && identityJson) {
  ...
  if (state && state.phase !== 'committed')
    throw new Error('Recover pending configuration transaction before deployment');
} else {
  throw new Error(
    'Usage: config-publication-cli.js prepare <config> <release-id> <identity-json> | verify <config>',
  );
}
