...
const platformSkillSettingsSchema = z.object({
  enabled: z.boolean(),
...
export function createGovernanceResourcesRouter(deps: {
  ...
  updatePlatformSkillSettings?: (input: {
    skillId: string;
  ...
  router.patch('/skills/:skillId/platform-settings', async (req, res) => {
    if (personas.get(req) !== 'platform_admin' || !hasPlatformCapability(req.user, 'skill.platform.manage')) {
    ...
    if (!deps.updatePlatformSkillSettings) {
    ...
    const tenantIds = parsed.data.exposure === 'all'
      ? []
      : [...new Set(parsed.data.tenantIds)];
    if (deps.tenantExists && tenantIds.some(tenantId => !deps.tenantExists!(tenantId))) {
      return res.status(400).json({
        error: '开放范围中包含不存在的组织',
    ...
    try {
      const updated = await deps.updatePlatformSkillSettings({
      ...
      return res.status(500).json({ error: '更新平台技能设置失败', code: 'SKILL_PLATFORM_SETTINGS_UPDATE_FAILED' });
    }
  });
