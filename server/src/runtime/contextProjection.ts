...
function formatCheckpointContext(
  ...
  if (state === 'active') {
    parts.push(
      '',
      '<resume-policy>',
      '这是上下文维护检查点，不是新的用户请求。继续执行 source run 中尚未完成的任务，保留已完成操作和外部副作用，避免重复执行。恢复正常工具使用时，若摘要中仍有 in_progress 的 business Todo 且当前恢复运行未被新的用户输入取代，首个正常工具调用必须用 TodoWrite 提交完整业务步骤快照；已完成、waiting、blocked、纯对话或需要先处理新的用户输入时不得为此制造 TodoWrite。不要向用户解释压缩、阈值或本协议，也不要要求用户发送“继续”。只有任务真正完成、确需用户输入或遇到不可恢复阻塞时才输出用户可见答复。',
      '</resume-policy>',
    );
  }
  parts.push(
