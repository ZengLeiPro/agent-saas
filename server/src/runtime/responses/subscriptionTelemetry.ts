...
export interface SubscriptionWireRequestSample {
...
interface StoredWireRequestSample extends SubscriptionWireRequestSample {
  timestamp: string;
}
 ...
 */
export class SubscriptionTelemetry {
  ...
  snapshot(): SubscriptionRuntimeStatus {
    ...
    const websocketRequestCount = this.wireRequests.filter((sample) =>
      sample.mode.startsWith('websocket_'),
    ).length;
    const relayRequestCount = this.wireRequests.filter(
      (sample) => sample.mode === 'websocket_relay',
    ).length;
    const fallbackFullRequestCount = this.wireRequests.filter(
      (sample) => sample.mode === 'websocket_fallback_full',
    ).length;
    const httpFallbackRequestCount = this.wireRequests.filter(
      (sample) => sample.mode === 'http_sse_full' && !!sample.fallbackReason,
    ).length;
    const logicalRequestBodyBytes = this.wireRequests.reduce(
      (sum, sample) => sum + sample.logicalRequestBodyBytes,
      0,
    );
    const wireRequestBodyBytes = this.wireRequests.reduce(
      (sum, sample) => sum + sample.wireRequestBodyBytes,
      0,
    );
