...
const describePg=url?describe:describe.skip; // Concurrency, migration, and crash tests require a real PostgreSQL transaction engine.
...
describePg('session automation real PostgreSQL integration',()=>{
 ...
 async function wake(key:string,generation=1){await store.tx(c=>store.scheduleTx(c,{tenantId:tenant,sessionId:session,automationId:automation,incarnationId:incarnation,generation,specVersion:1,continuationEpoch:1,triggerKey:key,dueAt:new Date(0),payload:{}}));}
