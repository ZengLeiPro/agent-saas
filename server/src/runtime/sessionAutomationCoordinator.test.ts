...
describe('SessionAutomationCoordinator execution gating and staged recovery', () => {
  it('continues cancellation, lifecycle, and staged recovery while new execution is disabled', async () => {
    const item = {
      cancellationId: 'cancel-1', tenantId: 'tenant-1', sessionId: 'session-1',
    ...
    expect(dispatcher.activate).toHaveBeenCalledWith('run-staged');
