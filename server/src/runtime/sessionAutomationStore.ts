...
export class PgSessionAutomationStore {
  ...
  async beginTerminalDrainLocked(client:Client,current:SessionAutomationSnapshot,desired:AutomationDesiredTerminalStatus,reason:string):Promise<SessionAutomationSnapshot>{
    ...
    await client.query(`UPDATE ${this.tables.automations}
      SET desired_terminal_status=$3,status=CASE WHEN $3='cancelled' THEN 'cancelling' ELSE 'completing' END,
  ...
  async tryFinalizeLocked(client:Client,tenantId:string,sessionId:string,automationId:string):Promise<SessionAutomationSnapshot|undefined>{
    ...
    if(decision.kind==='terminal')await client.query(`UPDATE ${this.tables.automations} SET status=$3,phase='terminal',active_run_id=NULL,next_wakeup_at=NULL,desired_terminal_status=NULL,projection_version=projection_version+1,updated_at=now() WHERE tenant_id=$1 AND automation_id=$2 AND incarnation_id=$4 AND generation=$5 AND desired_terminal_status=$3`,[tenantId,automationId,decision.status,row.incarnation_id,row.generation]);
  ...
  async completeCancellation(item:ClaimedCancellation):Promise<void>{await this.tx(async c=>{
    ...
    for(const row of target.rows){
      ...
      await c.query(`UPDATE ${this.tables.outbox} SET state=CASE WHEN state='completed' THEN state ELSE 'cancelled' END,lease_token=NULL,lease_expires_at=NULL,last_error=CASE WHEN state='completed' THEN last_error ELSE COALESCE(last_error,'run_cancelled') END WHERE outbox_id=$8 AND tenant_id=$2 AND session_id=$3 AND automation_id=$4 AND incarnation_id=$5 AND generation=$6 AND target_run_id=$7 AND state IN ('pending','dispatching','dispatched','dead')`,lineage);
