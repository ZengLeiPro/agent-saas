...
export class PgSessionAutomationStore {
 ...
 WHERE w.state='pending' AND w.due_at<=now() AND a.status='active' AND a.active_run_id IS NULL AND a.generation=w.generation AND a.incarnation_id=w.incarnation_id
