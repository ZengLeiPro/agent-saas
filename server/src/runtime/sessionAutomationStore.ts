...
export class PgSessionAutomationStore {
  ...
  async replace(client:Client,current:SessionAutomationSnapshot,spec:SessionAutomationSpec):Promise<SessionAutomationSnapshot>{
    ...
    await client.query(`UPDATE ${this.tables.automations} SET incarnation_id=$3,kind=$4,mode=$5,status='active',phase='waiting',active_run_id=NULL,generation=$6,spec_version=$7,control_version=control_version+1,projection_version=projection_version+1,continuation_epoch=1,updated_at=now() WHERE tenant_id=$1 AND automation_id=$2`,[current.tenantId,current.automationId,incarnationId,spec.kind,spec.mode,generation,specVersion]);
