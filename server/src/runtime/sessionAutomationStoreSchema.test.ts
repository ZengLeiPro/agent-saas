...
describe('session automation PG schema',()=>{
  ...
  it('defines complete lineage, idempotency and deferred ledger constraints',()=>{
    ...
    for(const table of [t.preparedDispatchAttempts,t.providerAttempts,t.budgetReservations,t.budgetSettlements,t.interactions,t.backgroundResources,t.reconciliationReceipts])expect(sql).toContain(`CREATE TABLE IF NOT EXISTS ${table}`);
    ...
    expect(sql).toContain("CHECK(state IN ('prepared','dispatched','completed','result_unknown','reconcile'))");
