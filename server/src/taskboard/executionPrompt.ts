...
export function executionWritebackInstructions(context: TaskboardExecutionContext): string[] {
  const instructions = [
    ...
    '- execution.context 默认只回传最近一条评论全文，更早的评论只给目录行（bodyTruncated=true，附 ordinal 与 bodyChars）；确需历史原文时用 comment.get({taskId, ordinal}) 或 comment.list({taskId, latest}) 按需取，不要整段回读全部评论。',
