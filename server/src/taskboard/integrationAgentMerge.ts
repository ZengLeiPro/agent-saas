...
export async function mergeIntegrationAgent(
  ...
  if (!host.repositoryProvider)
    throw new TaskboardValidationError(
      'Repository provider is unavailable',
  ...
  try {
    ...
    const loaded = await client.query(
      `SELECT t.*,b.repository,b.integration_policy,b.owner_user_id,
              ...
              a.provider_pull_request_id,a.integration_branch,a.review_head_oid,a.verdict,a.review_execution_id,
              a.admission_receipt,a.status AS agent_status,
              ...
              COALESCE((
                SELECT jsonb_agg(jsonb_build_object(
                  'id',source.id,
                  'deliveryTaskId',source.delivery_task_id,
                  'providerPullRequestId',source.provider_pull_request_id,
                  'reviewedSubjectDigest',source.reviewed_subject_digest,
                  'frozenHeadOid',source.frozen_head_oid,
                  'sourceOrder',source.source_order
                ) ORDER BY source.source_order,source.id)
                  FROM ${host.integrationSourcesTable} source
                 WHERE source.integration_task_id=t.id
              ),'[]'::jsonb) AS admission_sources
    ...
    if (
      ...
      row.purpose !== 'merge' ||
      !['queued', 'running', 'waiting_user', 'waiting_approval'].includes(
      ...
      throw new TaskboardValidationError(
        'Current execution cannot merge the integration Agent pull request',
        'TASKBOARD_INTEGRATION_AGENT_MERGE_INVALID',
      );
    }
    ...
    if (
      row.agent_status !== 'ready_to_merge' ||
      row.verdict !== 'approved' ||
      !reviewExecutionId ||
      row.review_purpose !== 'review' ||
      !row.review_transitioned_at ||
      !pullRequestId ||
      !reviewHeadOid
    ) {
      throw new TaskboardValidationError(
        'Integration Agent lacks a head-bound approved review',
        'TASKBOARD_INTEGRATION_AGENT_REVIEW_REQUIRED',
      );
    }
    ...
    const admissionSources = Array.isArray(row.admission_sources) ? row.admission_sources : [];
    const receiptValue = jsonObject(row.admission_receipt);
    const admissionTreeOid = String(receiptValue?.treeOid ?? '');
    const admissionBinding: IntegrationAdmissionBinding = {
      candidateId: String(row.id),
      candidateRevision: Number(row.version),
      reviewExecutionId,
      headOid: reviewHeadOid,
      baseOid: expectedBaseOid,
      treeOid: admissionTreeOid,
      subjectDigest: reviewedSubjectDigest,
      workflowEpoch: Number(row.workflow_epoch),
      laneEpoch: Number(row.lane_epoch),
      policyRevision,
      policyDigest: digestCanonical(policy ?? null),
      sourceSetDigest: digestCanonical(admissionSources),
    };
    const admission = verifyIntegrationAdmissionReceipt(receiptValue, admissionBinding);
    if (
      !policyRevision ||
      String(row.active_integration_task_id ?? '') !== String(row.id) ||
      admissionSources.length === 0 ||
      !expectedBaseOid ||
      !reviewedSubjectDigest ||
      !admissionTreeOid ||
      !admission.ok
    ) {
      throw new TaskboardValidationError(
        `Integration admission approval is stale: ${admission.ok ? 'binding is incomplete' : admission.reason}`,
        'TASKBOARD_INTEGRATION_ADMISSION_STALE',
      );
    }
    ...
    if (
      existingFence &&
      (existingFenceReviewExecutionId !== reviewExecutionId ||
        existingFenceReviewHeadOid !== reviewHeadOid)
    ) {
      throw new TaskboardValidationError(
        'Another Integration Agent merge is already in flight',
        'TASKBOARD_INTEGRATION_AGENT_MERGE_IN_FLIGHT',
      );
    }
    if (existingFence && existingFence !== executionId) {
      const fenceOwnerActive =
        Boolean(row.fence_owner_execution_id) &&
        ['queued', 'running', 'waiting_user', 'waiting_approval'].includes(
          String(row.fence_owner_execution_status),
        ) &&
        !row.fence_owner_transitioned_at &&
        !row.fence_owner_superseded_at;
      if (fenceOwnerActive) {
        throw new TaskboardValidationError(
          'Another Integration Agent merge is already in flight',
          'TASKBOARD_INTEGRATION_AGENT_MERGE_IN_FLIGHT',
    ...
    if (!repository || repository.provider !== 'github')
      throw new TaskboardValidationError(
        'Board repository is not configured',
        'TASKBOARD_REPOSITORY_REQUIRED',
      );
    const configured = repositoryWithBoardCiPolicy(
      repository as {
        provider: 'github';
        repositoryId: string;
        owner: string;
    ...
    const mergeMethod =
      configuredMethod === 'rebase' || configuredMethod === 'squash' ? configuredMethod : 'merge';
    if (
      inspectionPayload?.gateStatus !== 'success' ||
      inspectionReceipt?.executionId !== reviewExecutionId ||
      inspectionReceipt?.taskId !== String(row.id) ||
      inspectionReceipt?.purpose !== 'review' ||
      inspectionReceipt?.providerPullRequestId !== pullRequestId ||
      inspectionReceipt?.headOid !== reviewHeadOid ||
      reviewed?.providerPullRequestId !== pullRequestId ||
      reviewed?.headOid !== reviewHeadOid ||
      reviewed?.baseRef !== configured.baseBranch ||
      reviewed?.headRef !== String(row.integration_branch) ||
      !expectedBaseOid ||
      !reviewedSubjectDigest
    ) {
      throw new TaskboardValidationError(
        'Integration Agent approval lacks an exact reviewed revision',
        'TASKBOARD_INTEGRATION_AGENT_REVIEW_REQUIRED',
      );
    }
    if (!host.repositoryProvider.getCommit) {
      throw new TaskboardValidationError(
        'Repository provider cannot verify merge trees',
        'TASKBOARD_CI_UNAVAILABLE',
      );
    }
    const ownerUserId = String(row.owner_user_id);
    const reviewedCommit = await host.repositoryProvider.getCommit(
      configured,
      reviewHeadOid,
      ownerUserId,
    );
    if (
      reviewedCommit.oid !== reviewHeadOid ||
      reviewedCommit.treeOid !== admission.receipt.treeOid
    ) {
      throw new TaskboardValidationError(
        'Reviewed head tree could not be verified',
        'TASKBOARD_SUBJECT_STALE',
      );
    }
    const current = await host.repositoryProvider.getPullRequest(
      configured,
      pullRequestId,
      ownerUserId,
    );
    assertExactSubject(current, {
      ...
      headOid: reviewHeadOid,
      headRef: String(row.integration_branch),
      baseRef: configured.baseBranch,
      baseOid: expectedBaseOid,
      subjectDigest: reviewedSubjectDigest,
    });
    let receipt: RepositoryMergeReceipt | undefined;
    const operationKey = `integration-agent:${String(row.id)}:${admission.receipt.receiptDigest}`;
    if (current.state === 'merged') {
      ...
      if (!existingFence) {
        throw new TaskboardValidationError(
          'Merged pull request is not bound to a prior controlled merge attempt',
    ...
    if (!receipt)
      try {
        mergeAuthorityMayHaveCommitted = true;
        receipt = await host.repositoryProvider.mergePullRequest(
          configured,
          {
            providerPullRequestId: pullRequestId,
            expectedHeadOid: reviewHeadOid,
            method: mergeMethod,
        ...
        if (
          receipt.providerRequestId !== operationKey ||
          receipt.providerPullRequestId !== pullRequestId
        ) {
          throw new TaskboardValidationError(
            'Provider merge receipt is not bound to this request',
            'TASKBOARD_PROVIDER_RECEIPT_INCOMPLETE',
          );
        }
        mergedFacts = await host.repositoryProvider.getPullRequest(
          configured,
          pullRequestId,
          ownerUserId,
        );
        ...
        const afterError = await host.repositoryProvider.getPullRequest(
          configured,
          pullRequestId,
          ownerUserId,
        );
    ...
    if (!receipt.merged || !receipt.mergedCommitOid) {
      mergeAuthorityMayHaveCommitted = false;
      throw new TaskboardValidationError(
        receipt.message ?? 'Provider did not confirm merge',
        'TASKBOARD_PROVIDER_RECEIPT_INCOMPLETE',
      );
    }
    ...
    if (mergedFacts.state !== 'merged' || mergedFacts.mergeCommitOid !== receipt.mergedCommitOid) {
      throw new TaskboardValidationError(
        'Provider facts do not match the merge receipt',
        'TASKBOARD_PROVIDER_RECEIPT_INCOMPLETE',
      );
    }
    const mergedCommit = await host.repositoryProvider.getCommit(
      configured,
      receipt.mergedCommitOid,
      ownerUserId,
    );
    if (
      mergedCommit.oid !== receipt.mergedCommitOid ||
      mergedCommit.treeOid !== reviewedCommit.treeOid
    ) {
      throw new TaskboardValidationError(
        'Merged tree does not match the approved revision',
        'TASKBOARD_MERGE_RECEIPT_CONFLICT',
      );
    }
        ...
        JSON.stringify({
          ...
          raw: {
            ...
            approvedRevision: {
              baseOid: expectedBaseOid,
              headOid: reviewHeadOid,
              treeOid: reviewedCommit.treeOid,
            },
            admissionReceipt: admission.receipt,
            providerFacts: {
              providerPullRequestId: mergedFacts.providerPullRequestId,
              baseRef: mergedFacts.baseRef,
              baseOid: mergedFacts.baseOid,
              headRef: mergedFacts.headRef,
              headOid: mergedFacts.headOid,
              subjectDigest: mergedFacts.subjectDigest,
              state: mergedFacts.state,
              mergeCommitOid: mergedCommit.oid,
              mergedTreeOid: mergedCommit.treeOid,
            },
    ...
    if (!recorded.rows[0])
      throw new TaskboardValidationError(
        'Integration Agent merge fence changed before receipt persistence',
      ...
      await client
        .query(
          ...
          [
            String(row.id),
            String(row.execution_id),
            String(row.review_execution_id),
            String(row.review_head_oid),
          ],
        )
        .catch(() => undefined);
    }
...
function assertExactSubject(
  ...
  expected: Pick<
    RepositoryPullRequestSnapshot,
    'providerPullRequestId' | 'headOid' | 'headRef' | 'baseRef' | 'baseOid' | 'subjectDigest'
  >,
): void {
  if (
    actual.providerPullRequestId !== expected.providerPullRequestId ||
    actual.headOid !== expected.headOid ||
    actual.headRef !== expected.headRef ||
    actual.baseRef !== expected.baseRef ||
    actual.baseOid !== expected.baseOid ||
    actual.subjectDigest !== expected.subjectDigest
  ) {
    throw new TaskboardValidationError(
      'Integration Agent review is stale after pull request subject drift',
      'TASKBOARD_SUBJECT_STALE',
    );
  }
}
