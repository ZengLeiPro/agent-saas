...
const directoryProfileSchema = z.object({
  username: z.string().min(1), displayName: z.string().min(1), position: z.string().optional(),
...
export const membershipPreviewSchema = z.object({
  ...
  changeId: z.string().min(1), auditId: z.string().min(1).optional(),
}).strict();

const auditEventSchema = z.object({
  auditId: z.string(), correlationId: z.string().optional(), changeId: z.string().optional(),
  action: z.string(), result: z.string(), occurredAt: z.string(), actorUserId: z.string(),
  ...
  purpose: z.string().optional(), reason: z.string().optional(), metadata: z.record(z.string(), z.unknown()).optional(),
}).strict();
...
const resourceScopeSchema = z.object({
  resourceType: z.string(), mode: z.enum(['all', 'selected']), resourceIds: z.array(z.string()),
  source: z.string().optional(), version: z.number().int().positive(), allowedActions: z.array(actionSchema).optional(),
}).strict();
const policySchema = z.object({ policyKey: z.string(), value: z.unknown(), source: z.string(), version: z.number().int().positive() }).strict();
...
const previewTokenShape = {
  ...
  expiresAt: z.string().datetime({ offset: true }), changeId: z.string().min(1), auditId: z.string().optional(),
};
export const entitlementPreviewSchema = z.object({
  ...previewTokenShape,
  impact: z.object({ tenantId: z.string(), currentVersion: z.number().int().nonnegative(), nextVersion: z.number().int().positive(), fromStatus: z.string(), toStatus: z.string(), blockers: z.array(z.string()), reversible: z.boolean(), effectiveMode: z.string() }).strict(),
}).strict();
export const scopePreviewSchema = z.object({
  ...previewTokenShape,
  impact: z.object({ tenantId: z.string(), resourceType: z.string(), currentVersion: z.number().int().nonnegative(), nextVersion: z.number().int().positive(), from: z.object({ mode: z.string(), resourceCount: z.number().int().nonnegative() }).strict(), to: z.object({ mode: z.string(), resourceCount: z.number().int().nonnegative() }).strict(), blockers: z.array(z.string()), reversible: z.boolean(), effectiveMode: z.string() }).strict(),
}).strict();
...
export const lifecyclePreviewSchema = z.object({
  ...previewTokenShape,
  impact: z.object({ tenantId: z.string(), from: z.string(), to: z.string(), blockers: z.array(z.string()), reversible: z.boolean(), effectiveMode: z.string() }).strict(),
}).strict();
...
export const platformAdminListSchema = z.object({ platformAdmins: z.array(z.object({
  ...
  createdAt: z.string().optional(), updatedAt: z.string().optional(), directoryProfile: directoryProfileSchema.nullable().optional(),
}).strict()) }).strict();
...
const summarySchema = z.object({ status: z.string(), ids: z.array(z.string()).optional() }).strict();
export const offboardingPreviewSchema = z.object({
  ...
  expiresAt: z.string().datetime({ offset: true }), canCommit: z.boolean(),
  ...
  impact: z.object({
    ...
    cronOwnership: summarySchema, activeRuns: summarySchema.optional(), activeSessions: summarySchema.optional(),
    oauthGrants: summarySchema.optional(), externalConnections: summarySchema.optional(), personalMemory: summarySchema,
    fileOwnership: z.object({ status: z.string(), personalFileIds: z.array(z.string()).optional(), organizationFileIds: z.array(z.string()).optional() }).strict(),
  }).strict(),
}).strict();
export const changeJobSchema = z.object({
  ...
  }).strict()), created: z.boolean().optional(),
}).strict();
...
export const credentialListSchema = z.object({ credentials: z.array(z.object({
  ...
  custodianUserId: z.string().optional(), scopeSummary: credentialScopeSummarySchema.optional(), source: z.string().optional(),
  createdAt: z.string().optional(), createdBy: z.string().optional(), updatedAt: z.string().optional(), updatedBy: z.string().optional(),
}).strict()) }).strict();
...
export const memberDetailsSchema = z.object({
  ...
  usagePolicy: z.object({ status: z.literal('unavailable').optional(), tenantId: z.string().optional(), timezone: z.string().optional(), periodStart: z.string().optional(), periodEnd: z.string().optional(), monthUsedCreditsMicro: z.number().optional(), unattributedCreditsMicro: z.number().optional(), items: z.array(z.object({ userId: z.string(), monthlyLimitCreditsMicro: z.number().optional(), enforcementMode: z.string(), perRunLimitCreditsMicro: z.number().optional(), active: z.boolean(), version: z.number().int(), monthUsedCreditsMicro: z.number(), remainingCreditsMicro: z.number().optional(), canStartRun: z.boolean() }).strict()).optional() }).strict(),
