      ...
      {data && view === "profiles" && (
        <div className="grid min-h-0 flex-1 grid-rows-[190px_minmax(0,1fr)] gap-4 overflow-hidden md:grid-cols-[260px_minmax(0,1fr)] md:grid-rows-1">
          ...
          {selected && <Card className="min-h-0 overflow-hidden"><CardHeader className="border-b py-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><CardTitle className="text-base">{selected.name}</CardTitle><div className="mt-1 flex gap-2 text-xs text-muted-foreground"><span>{selected.profileKey}</span><span>修订 {selected.revision}</span>{selected.systemProfile && <span>系统预置</span>}{hasUnpublishedChanges && <span className="text-amber-700">有未发布修改</span>}</div></div><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={() => void createProfile(selected)} disabled={platformReadOnly || saving || !data.durable}><Copy className="size-3.5" />复制</Button><Button variant="outline" size="sm" onClick={() => void loadVersions()}><History className="size-3.5" />版本</Button>{!selected.systemProfile && <Button variant="outline" size="sm" onClick={() => void archive()} disabled={platformReadOnly || saving || !data.durable || selected.status === "archived"}><Archive className="size-3.5" />归档</Button>}<Button variant="outline" size="sm" onClick={() => void saveDraft()} disabled={platformReadOnly || saving || !data.durable || !dirty || selected.status === "archived"}><Save className="size-3.5" />保存草稿</Button><Button size="sm" onClick={() => void publish()} disabled={platformReadOnly || saving || !data.durable || selected.status === "archived" || (!configDirty && !!selected.latestVersion && !hasUnpublishedChanges)}><Rocket className="size-3.5" />发布新版</Button></div></div></CardHeader><CardContent className="h-[calc(100%-76px)] space-y-4 overflow-auto py-4">
            ...
            <div className="space-y-1.5"><div className="flex items-center justify-between"><Label>配置档 JSON</Label><span className="text-xs text-muted-foreground">schemaVersion 1 · 后端严格校验</span></div><Textarea value={configText} onChange={(event) => setConfigText(event.target.value)} className="min-h-[420px] font-mono text-xs" spellCheck={false} disabled={platformReadOnly || selected.status === "archived"} /></div>
            <EffectiveSummary config={safeConfig(configText) ?? selected.draftConfig} platformTools={data.platformTools} />
          </CardContent></Card>}
        </div>
      )}

      {data && view === "bindings" && <Card className="min-h-0 flex-1 overflow-auto"><CardHeader><CardTitle className="text-base">运行入口绑定</CardTitle><p className="text-sm text-muted-foreground">绑定稳定配置档；发布新版后只影响后续新会话，已存在会话继续使用自己的不可变版本。</p></CardHeader><CardContent className="space-y-3">{data.bindingKeys.map((key) => { const binding = data.bindings.find((item) => item.bindingKey === key); return <div key={key} className="grid items-center gap-2 rounded-lg border p-3 md:grid-cols-[220px_minmax(0,1fr)_180px]"><div><div className="text-sm font-medium">{BINDING_LABELS[key] ?? "其他运行入口"}</div><div className="font-mono text-xs text-muted-foreground">{key}</div></div><select className="h-9 rounded-md border bg-background px-3 text-sm" value={binding?.profileId ?? ""} disabled={platformReadOnly || saving || !data.durable} onChange={(event) => void updateBinding(key, event.target.value)}><option value="" disabled>请选择已发布配置档</option>{data.profiles.filter((profile) => profile.status === "published" && profile.latestVersion).map((profile) => <option key={profile.profileId} value={profile.profileId}>{profile.name} · v{profile.latestVersion!.versionNumber}</option>)}</select><div className="text-xs text-muted-foreground">{binding ? `${formatTime(binding.updatedAt)} · ${binding.updatedBy}` : "未绑定（运行时使用内置兼容版本）"}</div></div>; })}</CardContent></Card>}

      {data && view === "versions" && <div className="grid min-h-0 flex-1 gap-4 overflow-hidden md:grid-cols-[260px_minmax(0,1fr)]"><Card className="min-h-0 overflow-auto"><CardContent className="p-2">{versions.length === 0 ? <div className="p-4 text-sm text-muted-foreground">当前配置档尚无已发布版本</div> : versions.map((version) => <button key={version.profileVersionId} type="button" onClick={() => setSelectedVersionId(version.profileVersionId)} className={cn("mb-1 w-full rounded-lg px-3 py-2 text-left", version.profileVersionId === selectedVersion?.profileVersionId ? "bg-brand-accent-soft" : "hover:bg-muted")}><div className="text-sm font-medium">版本 {version.versionNumber}</div><div className="mt-1 text-xs text-muted-foreground">{formatTime(version.publishedAt)} · {version.publishedBy}</div><div className="mt-1 truncate font-mono text-[10px] text-muted-foreground">{version.configDigest}</div></button>)}</CardContent></Card><Card className="min-h-0 overflow-auto"><CardHeader><CardTitle className="text-base">{selectedVersion ? `${selected?.name ?? "配置档"} · v${selectedVersion.versionNumber}` : "版本详情"}</CardTitle><p className="text-sm text-muted-foreground">已发布版本只读且不可变；数据库触发器同时禁止更新和删除。</p></CardHeader><CardContent>{selectedVersion ? <><EffectiveSummary config={selectedVersion.config} platformTools={data.platformTools} /><pre className="mt-4 overflow-auto rounded-lg bg-muted p-4 text-xs leading-5">{JSON.stringify(selectedVersion.config, null, 2)}</pre></> : <div className="text-sm text-muted-foreground">选择一个版本查看</div>}</CardContent></Card></div>}
    </div>
  );
}
