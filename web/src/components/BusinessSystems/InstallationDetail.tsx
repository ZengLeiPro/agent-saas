    ...
    <section className="space-y-4 p-4">
      ...
      {!detail ? (
        ...
        <>
          ...
          <p className="text-sm">
            {detail.installation.installationId} · 组织 {detail.installation.tenantId} ·{' '}
          ...
          <dl className="grid grid-cols-2 gap-2 text-sm">
            ...
            <dt>域名验证</dt>
            <dd>{detail.installation.domainVerifiedAt ?? '待验证'}</dd>
          </dl>
          {detail.domainVerification && !detail.installation.domainVerifiedAt && (
            <div className="rounded border p-3 text-xs">
              <p>请技术联系人配置 DNS TXT：</p>
              <p className="break-all">{detail.domainVerification.recordName}</p>
              <p className="break-all">{detail.domainVerification.recordValue}</p>
            </div>
          )}
          <p className="text-sm">
            组织授权范围：{detail.assignmentSummary?.configured ? '已配置' : '待配置'}
          </p>
              ...
              ([action, label]) =>
                actions.includes(action!) && (
                  <Button
                    ...
                    disabled={busy}
                    onClick={() =>
                      void status(action === 'verify_domain' ? 'verify-domain' : action!)
                    }
                  >
                    {label}
                  </Button>
          ...
          {['delivery', 'usage', 'audit'].includes(tab) && (
            <InstallationReadPanel
              key={tab}
              installationId={installationId}
              suffix={tab}
              title={tab === 'delivery' ? '交付清单' : tab === 'usage' ? '实例用量' : '操作审计'}
            />
          )}
        </>
      )}
