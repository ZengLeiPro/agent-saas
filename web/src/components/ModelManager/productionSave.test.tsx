...
const models = {
  ...
  groups: [
    { id: 'main', name: 'Main', models: [{ id: 'gpt', name: 'GPT', value: 'original-model' }] },
  ],
};
const view = {
  revision: 'current-production-revision',
  models,
  memoryIndex: null,
...
beforeEach(() => {
  ...
  vi.mocked(authFetch).mockImplementation(async (_path, init) =>
    init?.method === 'PUT'
      ? json({
          ...view,
          revision: 'new-production-revision',
...
describe('production ModelManager save', () => {
  it('saves an editable production configuration with exact-revision confirmation', async () => {
    ...
    fireEvent.change(screen.getByDisplayValue('original-model'), {
