...
function canResetMemberPassword(
  member: MembershipRecord,
  currentUserId: string | undefined,
  isAdmin: boolean,
  isPlatformAdmin: boolean,
): boolean {
  return isPlatformAdmin || (isAdmin && (member.persona === "member" || member.userId === currentUserId));
}
...
function unresolvedOffboardingAuthority(preview: OffboardingPreviewResponse): string | null {
  if (!["clear", "transfer"].includes(preview.impact.cronOwnership.status)) return "定时任务归属状态未知或暂不可用";
  if (!["clear", "archive"].includes(preview.impact.personalMemory.status)) return "个人记忆状态未知或暂不可用";
  if (!["clear", "archive", "blocked"].includes(preview.impact.fileOwnership.status)) return "文件归属状态未知或暂不可用";
  return null;
}

function Loading() {
  return <div className="flex min-h-48 items-center justify-center text-sm text-muted-foreground">正在读取组织配置…</div>;
}
...
function OrganizationMemberDetails({ tenantId, userId, tab }: { tenantId: string; userId: string; tab: string }) {
  ...
  return <div className="space-y-5">
    ...
    {tab === "assignments" ? <div id="member-panel-assignments" role="tabpanel" className="grid gap-3 md:grid-cols-2">{data.assignments.map(group => <div key={group.resourceType} className="rounded-xl border bg-card p-4"><div className="flex items-center justify-between gap-3"><span className="font-medium">{group.resourceType}</span><Badge variant="outline">{group.resources.length} 项</Badge></div>{group.resources.length ? <ul className="mt-3 divide-y text-xs">{group.resources.map(resource => <li key={resource.resourceId} className="py-2"><div className="flex items-center justify-between gap-2"><span className="font-mono">{resource.resourceId}</span><Badge variant="secondary">已允许</Badge></div><div className="mt-1 text-muted-foreground">配置已同步</div><ul className="mt-2 space-y-1">{resource.bindings.map(binding => <li key={binding.assignmentId}>{binding.assigneeType}{binding.assigneeId ? `:${binding.assigneeId}` : ""} → {binding.effect}（{binding.origin}）</li>)}</ul></li>)}</ul> : <div className="mt-3 text-sm text-muted-foreground">无有效指派</div>}</div>)}</div> : null}
    {tab === "usage-policy" ? data.usagePolicy.status === "unavailable" ? <Empty text="用量策略暂不可用。" /> : memberUsage ? <div className="grid gap-3 rounded-xl border bg-card p-4 sm:grid-cols-4"><Fact label="统计周期" value={shanghaiNaturalMonth(data.usagePolicy.periodStart)} /><Fact label="成员本月已归属用量" value={String(memberUsage.monthAttributedCreditsMicro)} /><Fact label="个人月限额" value={memberUsage.monthlyLimitCreditsMicro === undefined ? "不限制" : String(memberUsage.monthlyLimitCreditsMicro)} /><Fact label="允许启动" value={memberUsage.canStartRun ? "是" : "否"} /></div> : <Empty text="当前成员的月用量明细不可用。" /> : null}
    {tab === "security-audit" ? <div id="member-panel-security-audit" role="tabpanel" className="space-y-4">
      <div className="rounded-xl border bg-card p-4"><div className="mb-3 font-medium">成员状态与身份</div><MembershipIdentityActions tenantId={tenantId} target={selected} onChanged={retry} /></div>
      <MemberAccountSecurity tenantId={tenantId} userId={userId} username={data.profile.username} displayName={data.profile.displayName} canResetPassword={canResetMemberPassword(selected, user?.id, isAdmin, isPlatformAdmin)} />
      <div className="text-xs text-muted-foreground">覆盖范围：最近 {data.recentAudit.limit} 条组织治理审计中的 Membership 端点事件</div>
      {data.recentAudit.events.length ? data.recentAudit.events.map(event => <div key={event.auditId} className="rounded-xl border bg-card p-4 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-medium">{event.action}</span><Badge variant="outline">{event.result}</Badge></div><div className="mt-2 text-xs text-muted-foreground">{new Date(event.occurredAt).toLocaleString()} · 操作人 {event.actorUserId}</div>{event.reason ? <div className="mt-2">原因：{event.reason}</div> : null}</div>) : <Empty text="当前覆盖窗口内没有成员治理记录。" />}
    </div> : null}
  </div>;
}
    ...
    {memberships.length === 0 ? <Empty text="当前组织没有可展示的治理成员关系。" /> : (
      <div className="overflow-x-auto rounded-xl border bg-card" tabIndex={0} aria-label="成员治理列表，可横向滚动">
        <table className="min-w-[900px] w-full text-sm"><thead className="bg-muted/50 text-left text-muted-foreground"><tr><th className="px-4 py-3">成员</th><th className="px-4 py-3">身份</th><th className="px-4 py-3">状态</th><th className="px-4 py-3">版本</th><th className="px-4 py-3">身份治理</th><th className="px-4 py-3">详情</th></tr></thead>
          <tbody className="divide-y">{memberships.filter(item => route.routeId !== "organization.members.owners" || item.persona === "org_admin").map(item => <tr key={item.userId}><td className="px-4 py-3"><div className="font-medium">{item.directoryProfile?.displayName ?? "目录资料不可用"}</div><div className="text-xs text-muted-foreground">{item.directoryProfile?.username ?? "账号未知"} · <span className="font-mono">{item.userId}</span></div></td><td className="px-4 py-3"><span className="inline-flex items-center gap-2">{personaLabel[item.persona]}{item.isOwner && <Badge variant="outline">所有者</Badge>}</span></td><td className="px-4 py-3"><Badge variant={item.status === "active" ? "secondary" : "outline"}>{localizedValue(item.status)}</Badge></td><td className="px-4 py-3">v{item.version}</td><td className="px-4 py-3"><MembershipIdentityActions tenantId={tenantId} target={item} onChanged={retry} /></td><td className="px-4 py-3"><div className="flex items-center gap-2">{canResetMemberPassword(item, user?.id, isAdmin, isPlatformAdmin) ? <LazyMemberPasswordResetAction userId={item.userId} displayName={item.directoryProfile?.displayName ?? item.directoryProfile?.username ?? item.userId} buttonLabel={`重置 ${item.directoryProfile?.displayName ?? item.directoryProfile?.username ?? item.userId} 的密码`} size="sm" /> : null}<Button size="sm" variant="outline" onClick={() => navigateGovernance(governanceRoute("organization.members.member", { orgId: tenantId, entityId: item.userId, tab: "profile", search: route.search }))}>打开详情</Button></div></td></tr>)}</tbody>
        </table>
      </div>
    )}
...
export function OrganizationGroupsPage({ tenantId }: { tenantId: string }) {
  ...
  return <div>
    ...
    {groups.length ? <div className="overflow-x-auto rounded-xl border bg-card" tabIndex={0} aria-label="目录群组列表，可横向滚动"><table className="min-w-[720px] w-full text-sm"><thead className="bg-muted/50 text-left text-muted-foreground"><tr><th className="px-4 py-3">群组</th><th className="px-4 py-3">本地群组 ID</th><th className="px-4 py-3">来源</th><th className="px-4 py-3">状态</th><th className="px-4 py-3">版本</th></tr></thead><tbody className="divide-y">{groups.map(group => <tr key={group.groupId}><td className="px-4 py-3">{group.displayName}</td><td className="px-4 py-3 font-mono text-xs">{group.groupId}</td><td className="px-4 py-3">{localizedValue(group.source, sourceLabels)}</td><td className="px-4 py-3"><Badge variant={group.status === "active" ? "secondary" : "outline"}>{localizedValue(group.status)}</Badge></td><td className="px-4 py-3">v{group.version}</td></tr>)}</tbody></table></div> : <Empty text="当前组织尚无同步群组。" />}
...
export function OrganizationOffboardingPage({ tenantId }: { tenantId: string }) {
  ...
  const commit = async () => {
    ...
    if (unresolvedAuthority) {
      setError(`${unresolvedAuthority}，为避免遗漏授权，当前不能提交。请刷新影响预览后重试。`);
      return false;
    }
  ...
  useSettingsDirtyEntry({
    ...
    save: async () => {
      if (!preview) { setError("请先生成并确认变更影响预览，再保存并离开。"); throw new Error("Offboarding preview required"); }
...
export function OrganizationCredentialsPage({ tenantId }: { tenantId: string }) {
  ...
  return <div><SectionTitle title="连接器与凭据" description="展示目录、凭据状态与健康结果；密钥仅写入安全存储，响应与页面永不回显。" />
...
export function OrganizationMemoryKnowledgePage({ tenantId }: { tenantId: string }) {
  ...
  return (
    <Tabs value={locationState.view} onValueChange={value => navigate(value as MemoryKnowledgeView)} className="min-h-0">
      <TabsList aria-label="记忆与知识区域" className={PAGE_TABS_LIST_CLASS}>
        <TabsTrigger value="governance" className={PAGE_TAB_TRIGGER_CLASS}>资源治理</TabsTrigger>
        <TabsTrigger value="context-center" className={PAGE_TAB_TRIGGER_CLASS}>Context Center</TabsTrigger>
        <TabsTrigger value="timeline" className={PAGE_TAB_TRIGGER_CLASS}>Timeline</TabsTrigger>
        <TabsTrigger value="entities" className={PAGE_TAB_TRIGGER_CLASS}>实体</TabsTrigger>
        <TabsTrigger value="reviews" className={PAGE_TAB_TRIGGER_CLASS}>待审核</TabsTrigger>
      </TabsList>
...
export function OrganizationEnvironmentsPage({ tenantId }: { tenantId: string }) {
  ...
  if (loading) return <Loading />; if (error) return <GovernanceUnavailable error={error} onRetry={retry} />;
  return <div><SectionTitle title="环境可用范围" description="展示已发布环境模板和本组织当前可用范围；修改前会先展示影响预览。" /><div className="space-y-4"><OrganizationEntitlementScopeEditor tenantId={tenantId} resourceType="environment_template" title="环境模板可用范围" description="当前目录资源与已退出目录的历史引用使用统一范围编辑规则。" onChanged={retry} /><OrganizationAssignmentManager tenantId={tenantId} resourceType="environment_template" title="环境模板成员与群组授权" items={(data?.templates ?? []).map(item => ({ resourceId: item.templateId, label: item.name }))} /></div></div>;
}

export function OrganizationGovernancePlaceholder({ title, detail }: { title: string; detail: string }) {
  return <div><SectionTitle title={title} description={detail} /><div className="rounded-xl border border-dashed bg-muted/10 p-8 text-center"><RefreshCw className="mx-auto size-6 text-muted-foreground" /><p className="mt-3 text-sm text-muted-foreground">相关服务或影响预览尚未接入，本页不会展示模拟数据或虚假成功。</p></div></div>;
}
