...
function OrganizationMemberDetails({ tenantId, userId, tab }: { tenantId: string; userId: string; tab: string }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="rounded-xl border bg-card p-5">
      ...
      <div className="mt-3 text-xs text-muted-foreground">Membership v{selected.version}{selected.updatedAt ? ` · ${new Date(selected.updatedAt).toLocaleString()}` : ""} · 快照 {new Date(data.snapshot.generatedAt).toLocaleString()}</div>
    </div>
    ...
    {tab === "access" ? <div id="member-panel-access" role="tabpanel" aria-labelledby="member-tab-access" className="space-y-3"><div className="grid gap-3 sm:grid-cols-3"><Fact label="最终身份" value={personaLabel[data.accessSummary.effectivePersona]} /><Fact label="Owner" value={data.accessSummary.owner ? "是" : "否"} /><Fact label="账号判定" value={data.accessSummary.decision === "eligible" ? "可参与权限解析" : "已拒绝"} /></div><div className="rounded-xl border bg-card p-4"><div className="font-medium">为什么</div><ul className="mt-2 divide-y text-sm">{data.accessSummary.why.map((item, index) => <li key={`${item.source}-${index}`} className="flex justify-between gap-3 py-2"><span>{item.source}</span><span className="text-muted-foreground">{item.effect} · v{item.version}</span></li>)}</ul></div></div> : null}
    ...
    {memberships.length === 0 ? <Empty text="当前组织没有可展示的治理 Membership。" /> : (
      <div className="overflow-x-auto rounded-xl border bg-card" tabIndex={0} aria-label="成员治理列表，可横向滚动">
        <table className="min-w-[900px] w-full text-sm"><thead className="bg-muted/50 text-left text-muted-foreground"><tr><th className="px-4 py-3">成员</th><th className="px-4 py-3">身份</th><th className="px-4 py-3">状态</th><th className="px-4 py-3">版本</th><th className="px-4 py-3">身份治理</th><th className="px-4 py-3">详情</th></tr></thead>
          <tbody className="divide-y">{memberships.filter(item => route.routeId !== "organization.members.owners" || item.persona === "org_admin").map(item => <tr key={item.userId}><td className="px-4 py-3"><div className="font-medium">{item.directoryProfile?.displayName ?? "目录资料不可用"}</div><div className="text-xs text-muted-foreground">{item.directoryProfile?.username ?? "账号未知"} · <span className="font-mono">{item.userId}</span></div></td><td className="px-4 py-3"><span className="inline-flex items-center gap-2">{personaLabel[item.persona]}{item.isOwner && <Badge variant="outline">Owner</Badge>}</span></td><td className="px-4 py-3"><Badge variant={item.status === "active" ? "secondary" : "outline"}>{item.status}</Badge></td><td className="px-4 py-3">v{item.version}</td><td className="px-4 py-3"><MembershipIdentityActions tenantId={tenantId} target={item} onChanged={retry} /></td><td className="px-4 py-3"><Button size="sm" variant="outline" onClick={() => navigateGovernance(governanceRoute("organization.members.member", { orgId: tenantId, entityId: item.userId, tab: "profile" }))}>打开详情</Button></td></tr>)}</tbody>
        </table>
      </div>
    )}
...
export function OrganizationGroupsPage({ tenantId }: { tenantId: string }) {
  ...
  return <div>
    <SectionTitle title="部门与群组" description="本地不可变 Group ID 承载治理绑定；外部目录 ID 仅作同步映射。" />
    {groups.length ? <div className="overflow-x-auto rounded-xl border bg-card" tabIndex={0} aria-label="目录群组列表，可横向滚动"><table className="min-w-[720px] w-full text-sm"><thead className="bg-muted/50 text-left text-muted-foreground"><tr><th className="px-4 py-3">群组</th><th className="px-4 py-3">本地 Group ID</th><th className="px-4 py-3">来源</th><th className="px-4 py-3">状态</th><th className="px-4 py-3">版本</th></tr></thead><tbody className="divide-y">{groups.map(group => <tr key={group.groupId}><td className="px-4 py-3">{group.displayName}</td><td className="px-4 py-3 font-mono text-xs">{group.groupId}</td><td className="px-4 py-3">{group.source}</td><td className="px-4 py-3"><Badge variant={group.status === "active" ? "secondary" : "outline"}>{group.status}</Badge></td><td className="px-4 py-3">v{group.version}</td></tr>)}</tbody></table></div> : <Empty text="目录权威服务已接入，但当前组织尚无同步群组。" />}
...
export function OrganizationOffboardingPage({ tenantId }: { tenantId: string }) {
  ...
  return <div className="space-y-5">
    ...
    {preview ? <div className="space-y-3 rounded-xl border bg-card p-5"><div className="flex items-center justify-between gap-3"><span className="font-medium">权威影响快照</span><Badge variant={preview.canCommit ? "secondary" : "destructive"}>{preview.canCommit ? "可提交" : `${preview.blockers.length} 个阻断`}</Badge></div><div className="grid gap-3 sm:grid-cols-3"><Fact label="组织 Agent 交接" value={`${preview.impact.agents.length} 项`} /><Fact label="个人 Agent 归档" value={`${preview.impact.personalAgents.length} 项`} /><Fact label="个人 Skill 停用保留" value={`${preview.impact.skills.length} 项`} /><Fact label="个人凭据撤销" value={`${preview.impact.personalCredentials.length} 项`} /><Fact label="凭据托管人转移" value={`${preview.impact.custodialCredentials.length} 项`} /><Fact label="定时任务" value={preview.impact.cronOwnership.status} /><Fact label="个人 Memory" value={preview.impact.personalMemory.status} /><Fact label="文件 ownership" value={preview.impact.fileOwnership.status} /><Fact label="活跃 Run" value={`${preview.impact.activeRuns?.ids?.length ?? 0} 项`} /><Fact label="Session 留存" value={`${preview.impact.activeSessions?.ids?.length ?? 0} 项`} /><Fact label="OAuth Grant" value={`${preview.impact.oauthGrants?.ids?.length ?? 0} 项`} /><Fact label="外部连接" value={`${preview.impact.externalConnections?.ids?.length ?? 0} 项`} /></div><div className="text-xs text-muted-foreground">基线 {preview.baselineDigest.slice(0, 12)}… · 签名预览有效期至 {new Date(preview.expiresAt).toLocaleString()}</div>{preview.blockers.length ? <ul className="space-y-1 rounded-xl border border-destructive/30 p-3 text-sm text-destructive">{preview.blockers.map(item => <li key={`${item.code}:${item.targetId ?? item.domain}`}>{item.domain} · {item.code}{item.targetId ? ` · ${item.targetId}` : ""}</li>)}</ul> : <Button disabled={busy || Date.parse(preview.expiresAt) <= Date.now()} onClick={() => void commit()}>确认交接并撤权</Button>}</div> : null}
    {result ? <div className="space-y-3 rounded-xl border p-4 text-sm">{receipt ? <div role="status" aria-live="polite" className="rounded-lg border bg-muted/30 p-3 text-xs"><div className="font-medium">治理回执</div><div>Change ID：{receipt.changeId ?? "未返回"} · Audit ID：{receipt.auditId ?? "未返回"}</div><div>{receipt.effectiveAt ? `生效于 ${new Date(receipt.effectiveAt).toLocaleString()}` : receipt.auditCompletion === "pending" ? `终态审计待投影${receipt.auditProjectionId ? ` · ${receipt.auditProjectionId}` : ""}` : "生效时间未返回"}</div></div> : null}<div className="flex flex-wrap items-center gap-2"><span>Change Job</span><strong>{result.job.status}</strong><span className="font-mono text-xs">{result.job.jobId}</span><Badge variant="outline">attempt {result.job.attempt}</Badge></div><div className="grid gap-2 md:grid-cols-2">{result.domains.map(domain => <div key={domain.domain} className="rounded-lg border p-3"><div className="flex items-center justify-between gap-2"><span className="font-medium">{domain.domain}</span><Badge variant={domain.status === "completed" ? "secondary" : "outline"}>{domain.status}</Badge></div><div className="mt-1 text-xs text-muted-foreground">完成 {domain.completedCount}/{domain.totalCount} · 失败 {domain.failedCount}{domain.lastErrorCode ? ` · ${domain.lastErrorCode}` : ""}</div>{domain.unresolvedItems?.length ? <ul className="mt-2 space-y-1 text-xs text-destructive">{domain.unresolvedItems.map(item => <li key={`${item.itemType}:${item.itemId}`}>{item.itemType} · {item.itemId} · {item.reasonCode}{item.retryable ? "（可重试）" : ""}</li>)}</ul> : null}</div>)}</div>{["retry_wait", "partial", "failed"].includes(result.job.status) ? <Button variant="outline" disabled={busy} onClick={() => void retryJob()}>重试未完成域</Button> : null}</div> : null}
  </div>;
}
