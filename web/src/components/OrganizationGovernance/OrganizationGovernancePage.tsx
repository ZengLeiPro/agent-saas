    ...
    {memberships.length === 0 ? <Empty text="当前组织没有可展示的治理成员关系。" /> : (
      <div className="overflow-x-auto rounded-xl border bg-card" tabIndex={0} aria-label="成员治理列表，可横向滚动">
        <table className="min-w-[900px] w-full text-sm"><thead className="bg-muted/50 text-left text-muted-foreground"><tr><th className="px-4 py-3">成员</th><th className="px-4 py-3">身份</th><th className="px-4 py-3">状态</th><th className="px-4 py-3">版本</th><th className="px-4 py-3">身份治理</th><th className="px-4 py-3">详情</th></tr></thead>
          <tbody className="divide-y">{memberships.filter(item => route.routeId !== "organization.members.owners" || item.persona === "org_admin").map(item => <tr key={item.userId}><td className="px-4 py-3"><div className="font-medium">{item.directoryProfile?.displayName ?? "目录资料不可用"}</div><div className="text-xs text-muted-foreground">{item.directoryProfile?.username ?? "账号未知"} · <span className="font-mono">{item.userId}</span></div></td><td className="px-4 py-3"><span className="inline-flex items-center gap-2">{personaLabel[item.persona]}{item.isOwner && <Badge variant="outline">所有者</Badge>}</span></td><td className="px-4 py-3"><Badge variant={item.status === "active" ? "secondary" : "outline"}>{localizedValue(item.status)}</Badge></td><td className="px-4 py-3">v{item.version}</td><td className="px-4 py-3"><MembershipIdentityActions tenantId={tenantId} target={item} onChanged={retry} /></td><td className="px-4 py-3"><div className="flex items-center gap-2">{canResetMemberPassword(item, user?.id, isAdmin, isPlatformAdmin) ? <MemberPasswordResetAction userId={item.userId} displayName={item.directoryProfile?.displayName ?? item.directoryProfile?.username ?? item.userId} buttonLabel={`重置 ${item.directoryProfile?.displayName ?? item.directoryProfile?.username ?? item.userId} 的密码`} size="sm" /> : null}<Button size="sm" variant="outline" onClick={() => navigateGovernance(governanceRoute("organization.members.member", { orgId: tenantId, entityId: item.userId, tab: "profile" }))}>打开详情</Button></div></td></tr>)}</tbody>
        </table>
      </div>
    )}
