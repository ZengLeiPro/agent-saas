...
describe("我的 Agent", () => {
  it("人格定义不再作为跳转 Tab，资料卡负责打开编辑弹窗", () => {
    ...
    expect(screen.queryByText("在资料与长期 Memory 之间切换；深链刷新会保留当前 Tab。")).toBeNull();
    fireEvent.pointerEnter(screen.getByRole("button", { name: "查看说明" }));
    expect(screen.getByRole("tooltip").textContent).toContain("在资料与长期 Memory 之间切换；深链刷新会保留当前 Tab。");
