...
function TenantLifecyclePanel({ tenantId }: { tenantId: string }) {
  ...
  return <div className="space-y-4">
    <div className="grid gap-3 sm:grid-cols-3"><Fact label="状态" value={data.status} /><Fact label="更新时间" value={new Date(data.updatedAt).toLocaleString()} /><Fact label="组织 ID" value={data.tenantId} /></div>
    ...
    {preview ? <div className="rounded-xl border bg-card p-4"><div className="text-sm">{preview.impact.from} → {preview.impact.to} · {preview.impact.reversible ? "可逆" : "不可逆"}</div><div className="mt-1 text-xs text-muted-foreground">生效方式：{preview.impact.effectiveMode} · 基线 {preview.baselineDigest.slice(0, 12)}… · 有效期至 {new Date(preview.expiresAt).toLocaleString()}</div>{preview.impact.blockers.length ? <div className="mt-2 rounded border border-destructive/30 p-2 text-xs text-destructive">阻断：{preview.impact.blockers.join("、")}</div> : null}<Button className="mt-3" disabled={busy || preview.impact.blockers.length > 0 || Date.parse(preview.expiresAt) <= Date.now()} onClick={() => void commit()}>确认执行</Button></div> : action ? <Button disabled={busy || reason.trim().length < 3} onClick={() => void runPreview()}>{action.label}</Button> : <Empty>后端未返回可执行动作。</Empty>}
  </div>;
}
...
function EntitlementActions({ tenantId, entitlement, actions, onChanged }: {
  ...
  return <div className="mt-4 space-y-3">
    ...
    {receipt ? <Receipt value={receipt} /> : null}
    {preview
      ? <div className="space-y-1 rounded-xl border p-4 text-sm"><div>影响预览：v{preview.impact.currentVersion} → v{preview.impact.nextVersion}</div><div>{preview.impact.fromStatus} → {preview.impact.toStatus} · {preview.impact.reversible ? "可逆" : "不可逆"}</div><div className="text-xs text-muted-foreground">生效方式：{preview.impact.effectiveMode} · 基线 {preview.baselineDigest.slice(0, 12)}… · 有效期至 {new Date(preview.expiresAt).toLocaleString()}</div>{preview.impact.blockers.length ? <div className="rounded border border-destructive/30 p-2 text-xs text-destructive">阻断：{preview.impact.blockers.join("、")}</div> : null}<Button className="mt-3" disabled={busy || preview.impact.blockers.length > 0 || Date.parse(preview.expiresAt) <= Date.now()} onClick={() => void commit()}>确认执行</Button></div>
...
function ScopeEditor({ tenantId, scope, onChanged }: { tenantId: string; scope: ResourceScope; onChanged: () => void }) {
  ...
  return <div className="mt-3 space-y-3 rounded-lg border p-3">
    ...
    {receipt ? <Receipt value={receipt} /> : null}
    {preview ? <div className="space-y-1 rounded-lg bg-muted p-3 text-xs"><div>v{preview.impact.currentVersion} → v{preview.impact.nextVersion} · {preview.impact.from.mode}({preview.impact.from.resourceCount}) → {preview.impact.to.mode}({preview.impact.to.resourceCount})</div><div>生效方式：{preview.impact.effectiveMode} · {preview.impact.reversible ? "可逆" : "不可逆"} · 基线 {preview.baselineDigest.slice(0, 12)}… · 有效期至 {new Date(preview.expiresAt).toLocaleString()}</div>{preview.impact.blockers.length ? <div className="rounded border border-destructive/30 p-2 text-destructive">阻断：{preview.impact.blockers.join("、")}</div> : null}<Button className="mt-2" size="sm" disabled={busy || preview.impact.blockers.length > 0 || Date.parse(preview.expiresAt) <= Date.now()} onClick={() => void commit()}>确认提交</Button></div> : <Button size="sm" disabled={busy || staleIds.length > 0} onClick={() => void runPreview()}>预览变更</Button>}
  </div>;
}
...
export function PlatformOrganizationGovernance({ tenantId, route }: { tenantId: string; route: GovernanceRouteState }) {
  ...
  if (tab === "resource-scope") return <div><Header title="资源范围" description="资源目录范围只接受全部允许或从目录选择，禁止手填 ID。" />
    {!data?.scopes.length ? <Empty>该组织尚无治理资源范围记录。</Empty> : <div className="grid gap-3 md:grid-cols-2">{data.scopes.map(scope => <div key={scope.resourceType} className="rounded-xl border bg-card p-4"><div className="flex items-center justify-between gap-3"><span className="font-medium">{scope.resourceType}</span><Badge variant="outline">{scope.mode === "all" ? "全部允许" : `已选 ${scope.resourceIds.length}`}</Badge></div><div className="mt-2 text-xs text-muted-foreground">{scope.source} · v{scope.version}</div>{scope.mode === "selected" && <div className="mt-3 break-all text-xs">{scope.resourceIds.join("、") || "没有已选资源"}</div>}<ScopeEditor tenantId={tenantId} scope={scope} onChanged={retry} /></div>)}</div>}
  </div>;
  ...
  if (tab === "security-lifecycle") return <div><Header title="安全与生命周期" description="组织暂停与恢复执行 preview→baseline 校验→审计回执；删除继续走 durable Change Job。" /><TenantLifecyclePanel tenantId={tenantId} /></div>;
...
export function PlatformAdminsPage() {
  ...
  return <div><Header title="平台管理员" description="该身份拥有完整平台控制面权限，不再展示失效的 capability 矩阵。" />
    {!data?.platformAdmins.length ? <Empty>没有可展示的平台管理员记录。</Empty> : <div className="overflow-x-auto rounded-xl border bg-card" tabIndex={0}><table className="min-w-[680px] w-full text-sm"><thead className="bg-muted/50 text-left text-muted-foreground"><tr><th className="px-4 py-3">平台管理员</th><th className="px-4 py-3">状态</th><th className="px-4 py-3">来源</th><th className="px-4 py-3">版本</th></tr></thead><tbody className="divide-y">{data.platformAdmins.map(item => <tr key={item.userId}><td className="px-4 py-3"><div className="font-medium">{item.directoryProfile?.displayName ?? "目录资料不可用"}</div><div className="text-xs text-muted-foreground">{item.directoryProfile?.username ?? "账号未知"} · <span className="font-mono">{item.userId}</span></div></td><td className="px-4 py-3"><Badge variant={item.status === "active" ? "secondary" : "outline"}>{item.status}</Badge></td><td className="px-4 py-3">{item.source}</td><td className="px-4 py-3">v{item.version}</td></tr>)}</tbody></table></div>}
...
export function PlatformTemplateCatalogPage({ kind }: { kind: "agent" | "environment" }) {
  ...
  return <div><Header title={kind === "agent" ? "Agent Template" : "Environment Template"} description="模板目录来自稳定资源与版本事实源；不使用展示字典或旧配置拼装。" />{items.length ? <div className="overflow-x-auto rounded-xl border bg-card" tabIndex={0}><table className="min-w-[680px] w-full text-sm"><thead className="bg-muted/50 text-left text-muted-foreground"><tr><th className="px-4 py-3">模板</th><th className="px-4 py-3">稳定 ID</th><th className="px-4 py-3">状态</th><th className="px-4 py-3">范围</th><th className="px-4 py-3">版本</th></tr></thead><tbody className="divide-y">{items.map(item => <tr key={item.id}><td className="px-4 py-3">{item.name}</td><td className="px-4 py-3 font-mono text-xs">{item.id}</td><td className="px-4 py-3"><Badge variant="outline">{item.status}</Badge></td><td className="px-4 py-3">{item.scope}</td><td className="px-4 py-3">v{item.version}</td></tr>)}</tbody></table></div> : <Empty>当前模板目录为空。</Empty>}</div>;
}
