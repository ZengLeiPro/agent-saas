...
export function SubagentTranscriptPanel({ childSessionId, title, onClose, hideHeader }: SubagentTranscriptPanelProps) {
  ...
  return (
    <RightPanelFrame
      title={`子任务完整过程 · ${title}`}
      subtitle={childSessionId}
      onClose={onClose}
      closeLabel="关闭子任务完整过程"
    >
      {content}
    </RightPanelFrame>
  );
