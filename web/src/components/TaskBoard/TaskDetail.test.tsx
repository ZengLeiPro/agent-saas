...
describe("TaskDetail 交互与草稿隔离", () => {
  ...
  it("任务详情使用统一响应式侧栏与品牌分段标签，并按评论数选择默认页", async () => { const user = userEvent.setup(); const onOpenChange = vi.fn(); render(<TaskDetail {...props({ onOpenChange })} />);
    ...
    const tabs = screen.getByRole("tablist", { name: "任务详情分区" }); expect(tabs.className).toContain("bg-card"); expect(tabs.className).toContain("h-12"); const information = screen.getByTestId("task-detail-information"); expect(information.getAttribute("aria-hidden")).toBe("false");
