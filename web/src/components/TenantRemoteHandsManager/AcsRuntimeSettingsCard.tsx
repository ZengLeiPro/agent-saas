...
export function AcsRuntimeSettingsCard({ readOnly }: { readOnly: boolean }) {
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      await save({ maxRunningSandboxes, warnRunningSandboxes, drainDeadlineMs });
    } catch (err) {
    ...
    <Card data-testid="acs-runtime-settings">
      <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0 pb-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Button size="sm" onClick={() => void handleSave()} disabled={readOnly || loading || saving || !config}>
        ...
        {loading && !config ? (
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="space-y-1.5">
              ...
              <Input id="acs-max-running" inputMode="numeric" value={maxRunningText} onChange={(event) => { setMaxRunningText(event.target.value); setLocalError(null); setDirty(true); }} disabled={readOnly || saving} />
              <p className="text-xs text-muted-foreground">0 表示不限制，最大 1000。</p>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="acs-warn-running">运行环境告警阈值</Label>
              <Input id="acs-warn-running" inputMode="numeric" value={warnRunningText} onChange={(event) => { setWarnRunningText(event.target.value); setLocalError(null); setDirty(true); }} disabled={readOnly || saving} />
              <p className="text-xs text-muted-foreground">达到阈值时触发运行容量告警。</p>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="acs-drain-deadline">排空超时（毫秒）</Label>
              <Input id="acs-drain-deadline" inputMode="numeric" value={drainDeadlineText} onChange={(event) => { setDrainDeadlineText(event.target.value); setLocalError(null); setDirty(true); }} disabled={readOnly || saving} />
