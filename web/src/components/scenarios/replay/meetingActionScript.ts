...
export const meetingActionScript: ReplayScript = {
      ...
      blocks: [
        {
          ...
          presentation: {
            title: "按事实 / 决定 / 行动三层抽取",
            detail: [
              { no: 1, text: "事实 18 条：营收 4.82 亿、毛利率 21.3%（同比 -2.1pt）、质量异议 37 起，全部带片段定位" },
              { no: 2, text: "决议 5 条：产能上调、中厚板投入、质量异议时限、双源供应、加计扣除口径" },
              { no: 3, text: "行动项 7 条：其中 5 条可直接从决议派生责任人与到期" },
              { no: 4, text: "缺口 2 项：决议④缺责任人、决议⑤缺时限——会上没说，我不替你补" },
              { no: 5, text: "争议 1 处：中厚板板块「停投入」还是「缓投入」，两位高管口径不一致，原样保留待裁决" },
            ],
            status: "warn",
            panel: [
              { op: "focus", view: "meeting" },
      ...
      blocks: [
        {
          ...
          durationMs: 2040,
          presentation: {
            title: "创建待办、发布纪要并向群播报",
            detail: [
              { k: "钉钉待办", v: "7 条 · 6 位责任人 · 逐条回读校验通过" },
              { k: "会议纪要", v: "MIN-2026Q2-OPS v1.0 已发布" },
              { tree: "├", k: "群播报", v: "跟进群 12 人 · 决议卡 1 张（标注含人工修改）" },
              { tree: "└", k: "到期提醒", v: "到期前 3 天提醒责任人，逾期升级至会议主持人" },
            ],
            status: "ok",
            receipt: { id: "MIN-2026Q2-OPS-v1.0", system: "会议记录版本库 + 钉钉待办", readBack: true },
            panel: [
              { op: "focus", view: "actions" },
        ...
        },
        {
          ...
          toolName: "TodoDispatch",
          toolId: "t-dispatch",
          content: "todos=7 accepted=7 minutes=v1.0 published groupBroadcast=1 readBack=ok",
        },
        {
          id: "m6-text",
          kind: "text",
        ...
        },
        {
          ...
          presentation: {
            ...
            detail: [
              ...
              { tree: "├", k: "适用判定", v: "本合同签署日 2026-08-04 ≥ 生效日 2026-08-01 → 适用 7 天" },
              { tree: "├", k: "出处", v: "2026 Q2 经营分析会 · 片段 T-0142 [01:08:12] · 质量总监 林知远" },
              { tree: "└", k: "同时带出", v: "生产副总 07-25 的范围修改：存量合同不追溯" },
            ],
            status: "ok",
            panel: [
              { op: "focus", view: "meeting" },
      ...
      blocks: [
        {
          ...
          presentation: {
            ...
            detail: [
              ...
              { no: 1, text: "会议记录库返回 v1.0，文件校验与发布版一致" },
              { no: 2, text: "决议登记簿 5 条编号均可解析，其中 1 条已有下游引用" },
              { no: 3, text: "钉钉待办 7 条状态为已接收，无重复创建" },
              { no: 4, text: "协同群播报 1 条，内容与 v1.0 决议表逐条比对一致" },
              { indent: 0, text: "→ 四方一致，无未决动作" },
            ],
            status: "ok",
            panel: [
              { op: "focus", view: "audit" },
  ...
  sources: [
    ...
    },
    {
      ...
      producer: "钉钉待办与群机器人（DWS 连接）",
      state: "needs-change",
      gap: "DWS 可创建待办、发群消息，但「待办 ↔ 决议编号」的反向索引与写后回读回执都不存在，到期提醒和逾期升级也没有产品化配置",
    },
    {
