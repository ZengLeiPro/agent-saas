...
export function useSessionAutomationRuntime({ sessionId, onSessionCommitted, onNotification }: UseSessionAutomationOptions) {
  ...
  const refresh = useCallback(async (targetSessionId = sessionIdRef.current) => {
    ...
    setError(null);
    try {
      const { fetchAutomationEvents, fetchSessionAutomation } = await import('@/lib/sessionAutomationApi');
  ...
  const submitCommand = useCallback(async (rawCommand: string, attachments: UploadedFile[]) => {
    ...
    setError(null);
    try {
      const { submitAutomationCommand } = await import('@/lib/sessionAutomationApi');
  ...
  const control = useCallback(async (request: AutomationControlRequest) => {
    ...
    setError(null);
    try {
      const { controlSessionAutomation } = await import('@/lib/sessionAutomationApi');
