...
describe("resolveChatRightPanelKind", () => {
  ...
  it("文件、Artifact 预览与子 Agent 的显式选择可以切换当前 slot", () => {
    expect(resolveChatRightPanelKind("preview", allOpen)).toBe("preview");
    expect(resolveChatRightPanelKind("artifact", allOpen)).toBe("artifact");
    expect(resolveChatRightPanelKind("subagent", allOpen)).toBe("subagent");
  });

  it("显式面板关闭后按确定性顺序回退，system 不会反向覆盖有效 intent", () => {
    expect(resolveChatRightPanelKind("business-step", { ...allOpen, businessStep: false, subagent: false, artifact: false, preview: false })).toBe("system");
    expect(resolveChatRightPanelKind(null, { ...allOpen, businessStep: false, subagent: false, artifact: false, preview: false })).toBe("system");
  });
