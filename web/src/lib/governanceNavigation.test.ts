...
describe("governance navigation registry", () => {
  it("只暴露平台/组织各五个工作区，并完整登记本地叶子与八个个人设置页", () => {
    ...
    expect(GOVERNANCE_NAVIGATION.platform.flatMap((item) => item.routes)).toHaveLength(23);
    expect(GOVERNANCE_NAVIGATION.organization.flatMap((item) => item.routes).filter((item) => item.navigation !== "detail")).toHaveLength(21);
