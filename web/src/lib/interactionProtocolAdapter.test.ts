...
describe('M20-05 Web interaction adapter', () => {
  it('sends the explicit protocol and prevents duplicate UI transitions through shared reducer', () => {
    ...
    expect(request).toMatchObject({ sessionId: 's', interactionId: 'i', response: { allow: true }, requestId: 'r' });
    ...
    state = reduceInteraction(state, webInteractionAckEvent({ type: 'respond_ok', interactionId: 'i', requestId: 'r', status: 'resolved' }, identity));
