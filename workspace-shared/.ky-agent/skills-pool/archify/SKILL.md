---
name: archify
description: 生成专业技术图表，包括系统架构图、云架构、安全边界、网络拓扑、工作流、审批流、Runbook、CI/CD、API 时序图、请求链路、数据流、ETL/ELT、PII 边界、数据血缘、状态机和生命周期图；可把自然语言或 Mermaid 草图重新布局为可导出的单文件 HTML/SVG/PNG 图。
license: MIT
metadata:
  version: "2.8"
  author: tt-a1i
...
## KY Agent Runtime Rules

- Put generated `.json`, `.html`, `.svg`, and exported image artifacts under `assets/yyyymmdd/` unless the user points to an existing project directory.
- Final `.html` files must be single-file and self-contained for the KY Agent preview sandbox: no CDN scripts, Google Fonts links, remote CSS, remote JS, or runtime image/font fetches. This fork removes the upstream Google Fonts links from `assets/template.html`; do not restore them.
- For Kaiyan or customer-facing material, use concise Chinese labels by default. Prefer Kaiyan blue `#2E56E1` and warm orange `#E8843A` only when they improve hierarchy; keep Archify's semantic component colors when they carry meaning.
- Run the renderer and artifact checker before delivery whenever shell access is available. Do not hand-edit generated HTML to bypass validation; fix the JSON and re-render.
