...
> **企业专家 DWS Broker**：当前会话提供 `DwsBusiness` 工具时，必须用它执行钉钉业务命令，禁止用 `Shell` 直接运行 `dws`。`args` 只传 `dws` 后面的 argv（不含 `dws`、`--profile`、`--format`、token）；`credentialMode=agent` 使用专家自身钉钉账号，`requester` 使用请求者已连接的唯一账号。写操作仍须先取得用户明确要求或确认并传 `confirmed=true`；本阶段不支持破坏性/高影响动作，也不通过 Broker 执行 `scripts/`、本地文件导入/导出/上传/下载或 `event +listen-im` / `event consume` 长连接监听。

> ⚠️ **平台运行时约定（必读，先于本文档其他章节）**：执行任何 `dws` 命令前必须读 [references/platform-runtime-context.md](./references/platform-runtime-context.md)。下面 6 条是平台集成硬规则：
>
> 1. **Token/config 隔离到 workspace**：`DWS_DISABLE_KEYCHAIN=1` + `DWS_CONFIG_DIR=/workspace/.dws/config` + `DWS_KEYCHAIN_DIR=/workspace/.dws/keys`。ACS sandbox 镜像已默认注入；本地开发需 `. .dws/env.sh` 或 `dws_runtime.dws_env()` 显式带 env。
> 2. **官方脚本依赖 env 继承**：`scripts/` 下 31 个官方 Python 脚本不内置 env 注入，ACS 容器里会继承镜像 ENV；本地开发必须先 source `.dws/env.sh`。`scripts/dws_runtime.py` 仅供自写脚本复用 workspace 路径规范，不 patch subprocess。
> 3. **SaaS 首次授权只走连接器页**：用户在「能力中心 → 连接器 → 钉钉」点击「连接钉钉」，平台负责 device flow。Agent 遇到未登录时只引导用户去连接器页，不自行用 Shell 启动 `dws auth login`。
> 4. **每 workspace 绑定独立钉钉账号**：日历、邮件、听记、考勤、日志等个人数据不得复用无人小号；机器人/自动化场景使用自定义应用模式。
> 5. **ACS 里可直接运行 `dws`**：容器 ENV 已注入，token 落在 `/workspace/.dws/keys/`；本地开发或裸机运行前必须先 source `.dws/env.sh` 或显式内联 env。
> 6. **职责边界**：实时钉钉产品操作走本 skill；批量历史聚合若当前会话启用了 `ky-data-query` 则优先走它；TTS、语音消息和长时间异步回复不属于 dws。
>
> **PAT 行为授权是预期机制，不是参数错误**：遇到 `PAT_*_RISK_NO_PERMISSION` 时不要盲目重试或改命令；先读取结构化错误的 `hint`、`actions` 与 `data.uri`（兼容旧字段 `authUrl` / `authorizationUrl`）。需要用户打开授权页时，URI 必须原样转交，并说明风险等级与授权时长；完整约定见 [platform-runtime-context.md](./references/platform-runtime-context.md) §7 和 [pat.md](./references/products/pat.md)。
>
> **平台默认部署**：本 skill 不携带任何用户 token/config；每个用户通过连接器页独立完成授权。平台镜像固定预装 `dingtalk-workspace-cli@1.0.60`，本 skill 同步自官方 `v1.0.60` mono 快照并叠加平台约定；不要在容器里运行 `dws skill setup/upgrade` 覆盖本 overlay。
>
> **如果调用失败**，先按这 6 条排查；仍不通时读完整 [references/platform-runtime-context.md](./references/platform-runtime-context.md) §6 故障表，再按官方结构化错误决定是否重试。
...
- **SaaS 实时事件边界**：同步 `DwsBusiness` Broker 暂不支持 `event +listen-im` 或 `event consume` 长连接。SaaS 只可执行 `event list/schema/status` 等有限读命令；收到监听、订阅、实时自动回复或事件驱动 Agent 的需求时，明确说明当前不可用，不用 Shell 绕过，也不要轮询消息历史或审批列表。平台外本地开发的监听说明见 [event.md](./references/products/event.md)。
...
| `event`           | 个人 IM/OA 事件目录、schema 与订阅状态查询；SaaS Broker 暂不支持实时长连接监听 | [event.md](./references/products/event.md)                     |
...
用户提到"查询个人 IM/OA 事件目录/查看事件字段/schema/查看订阅状态" → `event list` / `event schema` / `event status`
用户提到"监听有人@我/监听单聊或群消息/订阅 OA 审批事件/实时接收钉钉事件/监听并自动回复消息/驱动 Agent 处理消息" → SaaS 会话明确说明：同步 `DwsBusiness` Broker 暂不支持实时长连接监听；不得执行 `event +listen-im`、`event consume`，也不得改用 Shell 或轮询规避。仅平台外本地开发可按 event 参考文档使用长连接命令。
...
关键区分: oa(查询或操作审批) vs event(事件目录/schema/状态；SaaS 不承载审批事件长连接监听)
