...
本 skill 基于官方 **v1.0.45**（wukong-aligned）；skills-pool 维护「官方 baseline + 本文件 overlay」的组合快照，CLI 升级时由维护者手动 rsync 官方 tree 覆盖 + 保留本文件。**不要在容器里自动跑 `dws skill setup`**——那会用官方原始 skill 冲掉本 overlay。
