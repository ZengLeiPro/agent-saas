...
2. 使用 `向用户展示摘要并等待明确确认` 或返回待确认状态，等待用户明确确认
...
Example:
  # 开启打卡结果通知（Agent 调用时，必须先完成 向用户展示摘要并等待明确确认 二次确认，确认后再追加 --yes 执行）
  dws attendance selfsetting save --setting-scene checkResultNotify --user <USER_ID> --check-result-msg 1 --yes --format json
  # 更新极速打卡设置（Agent 调用时，必须先完成 向用户展示摘要并等待明确确认 二次确认，确认后再追加 --yes 执行）
  ...
  # 更新打卡提醒设置（Agent 调用时，必须先完成 向用户展示摘要并等待明确确认 二次确认，确认后再追加 --yes 执行）
...
Flags:
      ...
      --yes                                    用户已确认，跳过交互式确认提示
                                               Agent 调用时传入前必须已完成 向用户展示摘要并等待明确确认 二次确认
```
...
4. **展示待写入数据并等待确认**：向用户展示目标用户、设置场景、要更新的字段、当前值、新值和最终命令参数摘要。必须调用 `向用户展示摘要并等待明确确认` 或返回待确认状态，并等待用户明确确认。
...
禁止在未获得用户明确确认前执行保存；禁止为了推进流程自动添加全局 `--yes`。即使用户在最初需求中表达“直接改/不用问”，Agent 也必须先通过 `向用户展示摘要并等待明确确认` 或待确认状态展示完整参数摘要并获得明确确认后，才允许追加 `--yes` 执行。
...
- `selfsetting save` 是敏感写操作，AI 调用时必须先执行 `selfsetting get` 查询当前值，并向用户展示目标用户、设置场景、修改字段、“当前值 → 新值”和最终命令参数摘要；必须调用 `向用户展示摘要并等待明确确认` 或返回待确认状态等待用户明确确认；用户确认后才允许追加全局 `--yes` 执行保存。禁止未经确认直接执行或自动添加 `--yes`
