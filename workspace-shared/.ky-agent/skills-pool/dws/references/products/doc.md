...
**CRITICAL — 执行对应操作前，MUST 先用 当前环境的文件读取方式读取以下子文件：**
...
dws doc create --name "项目周报" --content-file assets/yyyymmdd/dws/x.md \
...
dws doc read --node <DOC_ID> --content-format jsonml --output assets/yyyymmdd/dws/doc.json   # 1. 读出当前 JSONML
# 修改 assets/yyyymmdd/dws/doc.json 中的 jsonml 数组
dws doc update --node <DOC_ID> --content-file assets/yyyymmdd/dws/doc.json \
...
dws doc export --node <NODE_ID> --output assets/yyyymmdd/dws/downloads/          # contentType=ALIDOC 走 export
dws doc download --node <NODE_ID> --output assets/yyyymmdd/dws/downloads/        # contentType≠ALIDOC 走 download
