...
1. `dws doc read --node <DOC_ID> --content-format jsonml --output assets/yyyymmdd/dws/doc.json` — 返回的 JSON 里有一个 `revision` 字段（比如 `42`）。
2. 编辑 `assets/yyyymmdd/dws/doc.json` 里的 `jsonml` 字段。
3. `dws doc update --node <DOC_ID> --content-file assets/yyyymmdd/dws/doc.json --content-format jsonml --mode overwrite --revision 42` — 如果文档在期间被改过，服务端返回 `VersionConflict`，此时重新执行第 1 步即可。
...
```bash
# 1. 把内容写入 UTF-8 文本文件：
#    Linux/Mac: assets/yyyymmdd/dws/<name>.md；Windows: %TEMP%\<name>.md
# 2. 一步写入：
...
```bash
# overwrite 整段（用户已确认）
dws doc update --node <DOC_ID> --content-file assets/yyyymmdd/dws/<name>.md --mode overwrite --content-format markdown

# append 末尾追加
dws doc update --node <DOC_ID> --content-file assets/yyyymmdd/dws/<name>-append.md --mode append --content-format markdown

# append 到指定 block 前（index 通过 doc block list 获取）
dws doc update --node <DOC_ID> --content-file assets/yyyymmdd/dws/<name>.md --mode append --index 2 --content-format markdown

# JSONML 整篇无损 overwrite（默认不做并发检查；并发敏感时加 --revision <N>）
dws doc update --node <DOC_ID> --content-file assets/yyyymmdd/dws/<name>.json --content-format jsonml --mode overwrite
