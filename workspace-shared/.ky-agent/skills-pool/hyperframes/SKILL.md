...
HyperFrames 内置字体里 **没有任何中文字体**（只有日文 noto-sans-jp），所以做中文视频必须**引入中文字体**。**首选直接用 Google Fonts 外链**（`<link>`/`@import` 引 Noto Sans SC 等）——lint 会报 `google_fonts_import`（一条静态源码规则、warning 级，不阻断渲染），忽略即可。若要产物自包含、渲染更快，可改走自带 `.woff2` + `@font-face`（可选优化）。两条路径都不走则中文落回 generic 字体 → 豆腐块/错误字形。完整方案（外链/自带两条路径、子集化、arm64 注意事项、实测清单）见 **[references/中文字体-开沿适配.md](references/中文字体-开沿适配.md)** —— 写中文 composition 前先读它。
