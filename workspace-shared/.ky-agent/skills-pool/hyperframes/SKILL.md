---
name: hyperframes
description: 用 HTML/CSS/GSAP 代码精确制作「确定性、可复现、数据驱动、可批量参数化」的 MP4 视频（HeyGen HyperFrames 框架）。适用场景：数据可视化视频（图表/排行榜/地图动画）、代码演示与 PR 走查视频、产品发布与功能介绍片、带 kinetic 字幕/语音旁白/场景转场的合成视频、用 --variables 批量渲染千变体图文卡（报价卡/产品卡/榜单卡）、网页或文档转视频。当用户要「用代码/精确可控地做视频」「把数据做成视频」「代码演示视频」「批量生成视频卡片」「确定性可复现的视频」「网页/文档转视频」时触发。⚠️做中文视频前必读 references/中文字体-开沿适配.md（内置零中文字体，需引入中文字体）。明确区别于：① 横滑网页 deck / PPT / 演讲页 → 用 pptx；② 给已有视频文件加/烧/翻译字幕 → 用 video-subtitle。CLI 命令、TTS/转写/抠图、各动画 adapter 等子模块见 references/modules/（详见 SKILL 末尾 Bundled Modules 段）。
---

# HyperFrames

> **Brand colors (Kaiyan)**: primary blue `#2E56E1`, accent orange `#E8843A`, orange text `#A0500E` (not `#B65E16`); Chinese font PingFang SC family. See `brand-guidelines` skill for the full spec.

HTML is the source of truth for video. A composition is an HTML file with `data-*` attributes for timing, a GSAP timeline for animation, and CSS for appearance. The framework handles clip visibility, media playback, and timeline sync.

## ACS Runtime Rules

HyperFrames now runs in an ACS Sandbox with a persistent per-user workspace, not on the old Mac-based Claude Code environment. Treat the workspace as shared across the user's sessions.

- Create each new project under `assets/yyyymmdd/<project-slug>/` unless the user points to an existing project. Put final MP4/WebM/MOV files, screenshots, reports, and review HTML under `assets/yyyymmdd/` with readable names.
- Use `uploads/` or explicit user-provided local paths as inputs. Copy source media into the project only when HyperFrames needs stable relative paths.
- Final deliverables must be self-contained: no CDN scripts, Google Fonts links, remote JS modules, or render-time remote font/model/texture fetches. Vendor required JS/fonts/assets into the project, or use dependencies already present in the runtime image.
- Do not rely on macOS apps, `open`, Homebrew, `localhost` being visible to the user, or shell job control such as `cmd &`, `kill %1`, or `lsof` cleanup. Use rendered files, screenshots, or the platform's preview/port mechanism when available.
- Do not install system packages from a skill invocation. If Python packages are missing, install only into the workspace `.venv`; if browser/FFmpeg/system libraries are missing, stop and report the image dependency gap.
- Avoid `--docker` in ACS unless the user explicitly asks for Docker-based reproduction. The sandbox is already the production runtime boundary.
...
npx hyperframes render --variables '{"title":"Q4 Report","theme":"dark"}' --output ../q4-report.mp4
...
- **Custom fonts:** If design.md names a font that isn't built-in, the user must provide `.woff2` files in a `fonts/` directory or you must vendor an open licensed font into the project. If missing, warn before writing HTML. When files exist, add `@font-face` declarations pointing to the local files.
- External media is acceptable only as source input during capture/preprocessing. Render-critical media in the final composition should be local project files; add `crossorigin="anonymous"` only when a temporary remote source is unavoidable during inspection.
- For dynamic text overflow, use `window.__hyperframes.fitTextFontSize(text, { maxWidth, fontFamily, fontWeight })`
- Project-local files live at the project root alongside `index.html`; sub-compositions use `../`. Final user-facing outputs still go to the date directory (`assets/yyyymmdd/`), not an untracked `renders/` folder.

### 中文字体（开沿必读 — 头号坑）

HyperFrames 内置字体里 **没有任何中文字体**（只有日文 noto-sans-jp），所以做中文视频必须**引入中文字体**。在 ACS 中默认走自带 `.woff2` + `@font-face`，不要用 Google Fonts 外链作为最终方案；外链字体只可作为临时探索，不可交付。两条路径都不走则中文落回 generic 字体 → 豆腐块/错误字形。完整方案（本地 woff2、子集化、实测清单、旁白 TTS）见 **[references/中文字体-开沿适配.md](references/中文字体-开沿适配.md)** —— 写中文 composition 前先读它。
...
After authoring animations, run the animation map to verify choreography. The script ships **inside this skill** at `scripts/animation-map.mjs`; run it by its **absolute path** (this SKILL.md's own directory + `/scripts/animation-map.mjs`). Do NOT use the repo-relative `skills/hyperframes/scripts/...` — that only resolves from the HyperFrames repo root, not in this pool/synced layout where your cwd is the project/workspace. (`animation-map.mjs` self-bootstraps `@hyperframes/producer` from npm on first run, so no repo install is needed.)

```bash
# 把 <SKILL_DIR> 替换为本 skill 的绝对目录（即本 SKILL.md 所在目录）
...
- **[references/transcript-guide.md](references/transcript-guide.md)** — Caption-side transcript handling: input formats, mandatory quality check, cleaning JS, OpenAI/Groq API fallback, "if no transcript exists" flow. (For the `transcribe` CLI invocation, model selection rules, and the `.en` gotcha, see `references/modules/hyperframes-media/SKILL.md`.)
...
## Bundled Modules（原 HyperFrames 子技能，已收敛为本 skill 的子模块，按需 Read）

下列内容原本是独立 skill，现统一收进 `references/modules/<name>/SKILL.md`，**按需 Read（它们不是独立 skill，无法用 `/<name>` 调用）**。
> ⚠️ 重要：本 SKILL 与各模块文档中出现的 `/gsap`、`the gsap skill`、`the hyperframes-cli skill`、`/three` 等任何「调用某 skill」措辞，一律理解为「Read `references/modules/<对应名>/SKILL.md`」。

- **CLI 开发循环** — `references/modules/hyperframes-cli/SKILL.md`：`npx hyperframes` 的 init/lint/inspect/preview/render/doctor 等命令与环境排障。
- **媒体预处理** — `references/modules/hyperframes-media/SKILL.md`：TTS（中文/中英混读用豆包·火山引擎脚本，英文用 Kokoro）/转写(Whisper)/抠图(u2net)。⚠️ 中文旁白默认走豆包脚本，别用 Kokoro 出中文；中文转写优先用开沿的 `audio-transcribe` skill（阿里云 fun-asr，中文更好）。
- **Catalog 安装** — `references/modules/hyperframes-registry/SKILL.md`：`hyperframes add <block/component>` 装可复用块并接入 index.html。
- **网页转视频** — `references/modules/website-to-hyperframes/SKILL.md`：给 URL 抓站做视频的 7 步工作流。
- **动画运行时 adapter**（写对应动画时再 Read）：
  - GSAP（主力 adapter）— `references/modules/gsap/SKILL.md`
  - Three.js / WebGL — `references/modules/three/SKILL.md`
  - Lottie — `references/modules/lottie/SKILL.md`
  - Anime.js — `references/modules/animejs/SKILL.md`
  - Web Animations API — `references/modules/waapi/SKILL.md`
  - CSS 动画 — `references/modules/css-animations/SKILL.md`
  - TypeGPU / WebGPU — `references/modules/typegpu/SKILL.md`
  - Tailwind — `references/modules/tailwind/SKILL.md`
