...
1. Generate options **deeply contextual to the user's prompt**. Every category — not just architectures — must reflect the specific product, brand, audience, and mood. Generic options that could appear on any picker are a failure.
   ...
   **Type pairings** (5-6) — **RUN the font discovery process from typography.md BEFORE generating pairings when network/local metadata is available.** Use its deterministic seed so the result is reproducible for this project. If metadata cannot be fetched in ACS, pick from local/vendored fonts or an explicit fallback set and say so. You will otherwise reach for the same 8 fonts every time (Bricolage Grotesque, Instrument Serif, Fraunces, Archivo Black, DM Serif Display, Space Grotesk, Fredoka) — that's your training data default, not a contextual choice. Match the brand's energy and audience. Cross-category per typography.md (never two sans-serifs). Final deliverables must use local `.woff2` or runtime fonts, not Google Fonts links.
...
4. Present the picker through the available platform preview mechanism. In ACS/headless sessions, do not assume a local browser opens and do not use shell job control (`python3 -m http.server ... &`, `kill %1`, `lsof`). If a port preview is available, start a managed static server for the current turn and share only the exposed URL after verifying HTTP 200. If no port preview is available, return the generated file path `.hyperframes/pick-design.html` and ask the caller/user to open it through the workspace file preview.
5. Once the user picks, tell them: "Copy the design.md from the picker and paste it here." The user pastes the markdown back into the conversation. Save it verbatim to `design.md` in the project root — it's already in spec format (YAML frontmatter + prose sections). Then proceed with construction.
