...
Pick from the inventory in [capabilities.md](capabilities.md) and implementation patterns in [techniques.md](../../../techniques.md). Examples of composable beats:
...
- **[techniques.md](../../../techniques.md)** — 13 primitive animation techniques with code patterns. Pick for beats, these are starting points to adapt, not templates to copy.
- **[text-effects.md](../../../text-effects.md)** — named text animation effects and implementation notes available inside the HyperFrames skill. Assign a specific effect ID to every headline, label, and copy element in every beat — not generic "fades in" descriptions.
...
**Global guardrails** — read [video-composition.md](../../../video-composition.md) first. It defines the medium rules: density, color presence, scale, frame composition, and how design.md is brand truth not layout spec. Then apply these capture-specific additions:
...
When planning beats, decide which ones deserve an HTML-in-Canvas treatment vs. a standard GSAP animation. If you want it, name it in the storyboard — Step 5 will read [`../../../html-in-canvas-patterns.md`](../../../html-in-canvas-patterns.md) for implementation. You don't need to specify the API details here.
...
Every text element in this beat must name a specific effect from the catalog. The reference page is at [`../../../text-effects.md`](../../../text-effects.md) from this module, or `hyperframes/references/text-effects.md` from the parent skill. It lists effect IDs and implementation notes; pick what fits the brand and this beat's mood — don't default to the same effect every beat.
...
At build time, use the HyperFrames text-effects reference. Do not assume a separate animate-text skill or legacy local skill asset directory exists in ACS. No creative decisions at build time — just spec retrieval and implementation.
