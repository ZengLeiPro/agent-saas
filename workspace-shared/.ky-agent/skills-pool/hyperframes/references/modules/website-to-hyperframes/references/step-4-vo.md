...
python3 <media>/scripts/doubao_tts.py narration.txt --voice cancan -o narration.mp3
```

豆包不返回词级时间戳，下一步用 `npx hyperframes transcribe narration.mp3 --language zh` 取时间轴。音色与参数见 hyperframes-media 模块。

**英文 / 其他语言旁白**：问用户选哪个 provider：
