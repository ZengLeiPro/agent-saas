...
中文 / 中英混读旁白走豆包脚本（见 `references/modules/hyperframes-media/SKILL.md`），写法上：

- 关键数字仍建议写中文读法避免歧义：`135+` → `一百三十五以上`、`99.9%` → `百分之九十九点九`。
- 豆包 `*_bigtts` 音色支持中英混读，`API` / `SaaS` / `AI Agent` 等术语**直接写原词**，不用像 Kokoro 那样拆成字母。
- 停顿用标点（逗号、句号、换行）控制；豆包不支持 SSML 标签，写了会被原样读出来。
