...
| 引擎 | 脚本 | 适用场景 | 依赖 |
|------|------|---------|------|
| GPT-Image-2（默认） | `scripts/generate_gpt_image_2.py` | 通用生图，OpenAI 最新图像模型，质量最高，支持文生图与以图生图。中英文 prompt 均可。多人共享时已做 workspace 级单飞队列与退避重试 | 平台 CLIProxyAPI 可达 + `CLIPROXY_API_KEY` 由 ACS secret/env 注入 |
...
| Gemini Web UI（备选）| `scripts/generate_browser.py` | gemini.google.com 浏览器模式，属于 legacy/兜底路径 | ACS 浏览器能力可用，且用户已授权登录态 |
...
3. **依赖平台 CLIProxyAPI 服务**：若 `CLIPROXY_BASE_URL` 不可达，脚本会报错并提示检查平台服务与 secret/env 配置
...
| 变量 | 默认值 |
|------|--------|
| `CLIPROXY_BASE_URL` | 未设置时回退到 `http://127.0.0.1:8317`，仅适用于平台已在容器内暴露 CLIProxyAPI 的场景 |
| `CLIPROXY_API_KEY` | 必填，由 ACS secret/env 注入 |
...
备选方案。该路径会访问 Gemini Web UI，依赖外部网页、浏览器登录态和网络可用性；只有当前 ACS 浏览器能力可用且用户确认授权登录态时使用。
...
- **引擎切换触发**：用户明确要求 21:9 / 4:5 等 gpt-image-2 不支持的严格宽高比 / 要 Gemini 额度 → 切 Gemini。GPT-Image-2 出现 `auth_unavailable`/503/504 等临时错误 → 先按脚本内置退避重试；重试后仍失败再报错并提示检查平台 CLIProxyAPI 与 secret/env，不要一失败就切走默认引擎
...
- 平台 CLIProxyAPI 服务可达（GPT-Image-2 引擎）
  - 环境变量 `CLIPROXY_BASE_URL` / `CLIPROXY_API_KEY`，其中 `CLIPROXY_API_KEY` 必须由 ACS secret/env 注入
