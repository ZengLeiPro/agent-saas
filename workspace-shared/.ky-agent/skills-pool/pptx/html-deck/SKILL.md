...
**两种风格共享**：横向翻页（键盘 ← →、滚轮、触屏、ESC 索引）、Lucide 图标、Motion One 入场动效。ACS 交付给用户的最终 HTML 必须单文件自包含；不要依赖 CDN、远程字体或相邻 JS/CSS 文件。
...
完成 deck 初稿后,若运行环境具备生图能力,主动问用户是否需要生成配图并插入 PPT。不要默认生成。按环境选择生图通道:

- **KY Agent / agent-saas 生产环境**:使用平台 `GenerateImage` 工具(gpt-image-2 / seedream 引擎)。若调用报错「未启用」,说明当前租户未授权生图,直接告知用户并跳过配图,不要另找绕路。
- **Codex 环境**:用 GPT-M 2.0 生成。
- 两者都没有:告知用户当前环境无生图能力,改用占位色块并注明后期补图。
...
两个 `template*.html` 是源模板——CSS、WebGL shader、翻页 JS 和 `<!-- SLIDES_HERE -->` 已预设。最终交付前必须移除/内联外部字体、图标、JS、图片等依赖，生成一个 `assets/yyyymmdd/<项目名>.html`。
...
1. 同时打开当前模板或生成页、正在修改的测试 PPT 做对照(skill 作者本机的原始参考 PPT 对本仓库使用者不可访问;本仓库的 Swiss 主题 golden source 就是 `assets/template-swiss.html` 加上 `references/swiss-layout-lock.md` 登记的 22 个版式)。
...
10. **图标用模板内置的 lucide-compatible helper**——写 `<i data-lucide="name"></i>` 即可,不要从 CDN 引入 lucide,也不要自己额外画散装 SVG
...
用当前环境可用的浏览器/Playwright 截图方式预览 `assets/yyyymmdd/<项目名>.html`；不要使用 macOS 专属 `open` 命令作为 ACS 主路径。
...
│   └── motion.min.js         ← Motion One 本地副本（可选增强,模板失败时会使用原生 WAAPI 兜底）
├── scripts/
...
**动效相关**:模板已把 Motion One/WAAPI 兜底加载和 recipe 逻辑内嵌到底部 module script。你不需要改 JS,只需要按 `layouts.md` / `layouts-swiss.md` 的骨架在 HTML 里加 `data-anim` / `data-animate` 即可。离线演示优先使用 `assets/motion.min.js`,缺失时自动降级为原生 WAAPI；再不支持动画时内容仍会 reveal,不依赖网络。风格 B 模板必须保留 `B` 键低功耗模式:切换后停止 WebGL/ASCII canvas RAF,取消正在运行的 Web Animations,并把当前页内容直接 reveal 到静态最终态。
