...
本主题的 golden source 是 `assets/template-swiss.html` 加上 `references/swiss-layout-lock.md` 登记的 22 个版式(作者本机的原始参考 PPT 对本仓库使用者不可访问)。
...
1. 同时打开当前 `assets/template-swiss.html` 和正在修改的测试 PPT 做对照(作者本机的原始参考 PPT 不可访问;本仓库的 Swiss golden source 就是这两份文件加上 `references/swiss-layout-lock.md` 登记的 22 个版式)。
...
**注意**:**不要自己额外画 SVG 图标**,用 `<i data-lucide="bookmark"></i>` 交给模板内置 helper 渲染；不要引入线上 lucide。
**示例代码**:
```html
<div class="cell-6">
  <div class="cell">
...
3. ❌ 自己额外画 SVG 图标或引 CDN → ✅ 用模板内置 `data-lucide` helper,棱角风格
