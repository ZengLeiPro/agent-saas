...
> **品牌色规范**：主蓝 `#2E56E1`、暖橙 `#E8843A`、暖橙文字 `#A0500E`（不用 `#B65E16`）。中文字体优先使用 ACS/Linux 可用的 `Noto Sans SC` / `Source Han Sans SC`，再 fallback 到微软雅黑、Heiti、PingFang。**注意**：历史曾把主蓝误写为 `#385FF5`，所有报价 PDF 一律用 `#2E56E1`。

为定制化业务客户生成 PDF 报价书。

## 第一步：信息采集（必须执行）

收到报价书需求后，**先分析用户已提供的信息，识别缺失项，使用当前环境可用的用户提问机制一次性提问**；如果没有专用提问工具，就直接在对话里一次性列出缺失项。不要边做边问，也不要在信息不全时就开始生成。
...
信息齐全后：
1. 组装 JSON 数据文件，写入 `assets/yyyymmdd/quotes/` 目录（8 位日期，无分隔符）
...
- `output_dir` = 输出目录，建议显式传入 `assets/yyyymmdd/quotes/`；未传时脚本默认写入当前工作区的 `assets/yyyymmdd/quotes/`（目录不存在时自动创建）

输出文件命名：`{客户名}_报价书.html` / `.pdf` / `.json`。脚本会清洗客户名中的路径分隔符、控制字符等危险字符；如果同名文件已存在，会自动追加时间戳后缀，避免覆盖历史报价书。
...
1. 优先让用户提供原 `.json` 文件；如果需要搜索历史文件，在 `assets/yyyymmdd/quotes/` 日期目录中定位，不能只按客户名全局模糊改
2. 如果搜索命中多个历史版本，必须让用户确认目标文件
3. 修改 JSON 内容后重新运行生成脚本；默认会生成新文件，不覆盖同名文件

## 解决方案文档工作流

当用户需要在报价书后附带方案文档时：

1. **来源是钉钉文档**：如果当前环境具备钉钉文档读取能力，用 dingtalk-docs 读取 Markdown → 保存为 `.md` 文件 → JSON 中 `solution.file` 指向该文件；若不可用，请用户提供导出的 Markdown 或源文件
2. **来源是本地文件**：用户上传文件从 `uploads/` 读取；如已有 `.md` 文件，复制或转换到与报价 JSON 同一 `assets/yyyymmdd/quotes/` 子目录后引用
3. **需要 AI 生成**：根据用户描述撰写 Markdown 方案文档 → 保存到与报价 JSON 同一目录 → 引用

方案 Markdown 支持的格式：标题（h1-h4）、有序/无序列表（含嵌套）、表格、加粗/斜体、链接、引用块、分割线。原始 HTML 会被转义；不要依赖 `<span style>` 等内联 HTML。图片只允许引用 Markdown 文件所在目录下的相对路径，并会 base64 内嵌；不要使用远程 URL、绝对路径或跳出目录的 `../` 路径。
...
- 生成依赖工作区 `.venv`/ACS 镜像提供的 Python `mistune`；PDF 渲染优先用 Node Playwright/Chromium，失败时降级到 Python `weasyprint`。如果依赖缺失，不要在用户任务中安装全局依赖，停止并报告镜像或项目依赖缺口。
- **中文字体兼容性（重要）**：模板 `body` 的 `font-family` 优先使用 Linux/ACS 常见的 `Noto Sans SC` / `Source Han Sans SC`，再 fallback 到 `Microsoft YaHei`、`Heiti SC`、`PingFang SC`。生成后可用 `pdftoppm` 渲染首页 PNG 肉眼检查中文，或用 `pypdf` 检查字体嵌入。
