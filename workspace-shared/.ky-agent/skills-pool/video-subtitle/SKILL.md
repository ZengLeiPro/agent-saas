...
```bash
# 完整流程：提取 + 翻译 + 烧录
python scripts/full.py video.mp4

# 仅提取字幕
python scripts/extract.py video.mp4

# 仅翻译字幕
python scripts/translate.py video.srt --to zh

# 仅烧录字幕
python scripts/burn.py video.mp4 video_zh.srt
```
...
- **ffmpeg**: 视频处理和字幕烧录（`brew install ffmpeg`）
- **Python 3.10+**: 运行脚本
- **mlx-whisper**: Apple Silicon 优化的语音识别（`pip3 install mlx-whisper`）
- **anthropic SDK**: AI 翻译（`pip3 install anthropic`）
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `Fontname` | Arial Unicode MS | 字体（需支持中文） |
...
**Whisper 依赖错误**: `brew reinstall protobuf`

**字幕显示乱码**: 使用支持中文的字体 `--style "Fontname=Arial Unicode MS"`
