---
name: xlsx
description: "Any task where a spreadsheet file (.xlsx, .xlsm, .csv, .tsv) is the primary input or output: open/read/edit/fix an existing file (columns, formulas, formatting, charts), create one from scratch or from other data, convert tabular formats, or clean messy tabular data (malformed rows, misplaced headers) into a proper spreadsheet. Trigger whenever the user references a spreadsheet file by name or path, even casually. Do NOT trigger when the deliverable is a Word document, HTML report, standalone Python script, or database pipeline, even if tabular data is involved."
