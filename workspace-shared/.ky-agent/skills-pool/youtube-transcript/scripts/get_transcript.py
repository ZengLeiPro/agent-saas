...
def get_transcript(video_id: str, with_timestamps: bool = False, languages: list[str] | None = None) -> str:
    """Fetch and format transcript for a YouTube video."""
    api = YouTubeTranscriptApi()
    if languages:
        transcript = api.fetch(video_id, languages=languages)
    else:
        # Try common languages in order: Chinese variants, then English
        for langs in [["zh", "zh-CN", "zh-Hans", "zh-Hant"], ["en"]]:
            try:
            ...
            # Last resort: let the API pick whatever is available
...
def main():
    ...
    parser.add_argument('--lang', '-l', nargs='+', default=None,
                        help='Language codes to try, e.g. --lang zh en')
    args = parser.parse_args()

    try:
        video_id = extract_video_id(args.video)
        transcript = get_transcript(video_id, with_timestamps=args.timestamps, languages=args.lang)
        print(transcript)
